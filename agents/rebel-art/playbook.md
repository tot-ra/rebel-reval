...
- Licensed authored fauna under `assets/animals/<vendor>/` should use one subfolder per species (for example `hendrik_reyneke/chicken/`, `hendrik_reyneke/goat/`), mirroring `assets/birds/<species>/`. Move each GLB with its Godot sidecars together, then update `assets/SOURCES.csv`, every relocated `*.import` `source_file`, and hard-coded paths in `tools/assets/**` plus fauna tests.
