...
- Licensed authored fauna under `assets/animals/<vendor>/`, modular facade GLBs under `assets/buildings/facades/`, and furniture or crafts GLBs under `assets/props/furniture/<stem>/` or `assets/props/crafts/<stem>/` should use one subfolder per model (for example `hendrik_reyneke/chicken/`, `facades/timber_window_open_shutters/`, `furniture/smithy_bed/smithy_bed.glb`), mirroring `assets/birds/<species>/`. Move each GLB with its Godot sidecars together, then update `assets/SOURCES.csv`, every relocated `*.import` `source_file`, and hard-coded paths in generators, showcase, evidence, and tests. For furniture, run `python3 tools/assets/relocate_furniture_per_model.py`, then fix `Path(...)` OUTPUT constants in the Blender generators. When bulk-rewriting paths programmatically, scope the file walk to the tree being moved and skip `.worktrees/` and `build/`.
