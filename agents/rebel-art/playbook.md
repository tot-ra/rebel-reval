...
- Licensed authored fauna under `assets/animals/<vendor>/` and modular facade GLBs under `assets/buildings/facades/` should use one subfolder per model (for example `hendrik_reyneke/chicken/`, `facades/timber_window_open_shutters/`), mirroring `assets/birds/<species>/`. Move each GLB with its Godot sidecars together, then update `assets/SOURCES.csv`, every relocated `*.import` `source_file`, and hard-coded paths in generators, showcase, evidence, and tests.
