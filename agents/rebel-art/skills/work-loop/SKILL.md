...
8. On success, release the claim and leave `- [~] + review: canon`. Create cross-role requests for runtime wiring, map placement, missing briefs, or research.
9. If blocked, release the claim, set a typed blocker, and create one request. Never hold GPU/tool work while waiting.
...
Art may maintain only `A-###` rows in its own backlog. Refill below three grounded open rows and cap at eight. Scope one asset, one shared animation set, or one coherent material correction per row. Cross-role needs use work requests.
