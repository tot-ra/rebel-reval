...
- Godot scans `build/` as `res://` (it is gitignored, not `.gdignore`d). A scratch copy of a `class_name` script left there (for example a pre-change backup) steals the global class cache, and every later run fails with "Class X hides a global script class". Keep backups outside the project or drop a `.gdignore` into the scratch folder; to recover, add the `.gdignore` and run `Godot --headless --editor --path . --quit-after 2` once to rebuild the cache.
