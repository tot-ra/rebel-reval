...
- `gabled_roof_mesh` and `tower_cone_roof_mesh` emit UVs in world or plate units. Set `uv1_scale` to repeats per unit (for example `ROOF_TILE_WORLD_DENSITY`), not to `building_uv_scale(..., size)`, which is meant for 0..1 BoxMesh faces. `CylinderMesh` side UVs cover only part of V, so use world triplanar or a custom mesh on drums and cones.
