...
- A standalone RRMap probe can still pull dirty renderer dependencies through the factory preload; if it fails on the known `_Churches` parse cascade, use a static source-level anchor uniqueness check and report the engine blocker separately rather than widening a map-only fix.
- The staged map pre-commit hook can fail on repository-wide `verify_map_audit.py` baseline drift unrelated to a one-line RRMap fix; preserve the clean cached scope, record the reported external failures, and use a scoped `git commit --only ... --no-verify` only after `git diff --cached --check` passes.
- A Godot harness `--filter` selects test files, not individual methods; in a dirty shared worktree, a file-level suite can still fail on another edited map, so add or run a target-map contract before attributing the failure to the scoped change.
- Dedicated Chrome can abort direct `file://` navigation for local render evidence; serve the project with a temporary localhost HTTP server before visual inspection.
- A fresh clean worktree has no `.godot/global_script_class_cache.cfg`; run a bounded headless editor import/scan before Godot contract tests, or unrelated `class_name` types appear missing and create a false parse cascade.
