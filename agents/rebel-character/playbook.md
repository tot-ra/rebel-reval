...
- A single-body rebuild fail-closes on shared-map pixel drift. Pass `--regenerate-shared` on `generate_hero_body.py` and `generate_character_lods.py` (or `share_character_textures.py`) only when you intend to rewrite `assets/characters/shared/textures/`. `git checkout HEAD --` of that folder is a recovery step, not the guard.
- A Blender generator that must expose `--help` under plain `python3` has to parse argv and exit before `import bpy`. An import at the top of the file makes `--help` fail with ModuleNotFoundError.
