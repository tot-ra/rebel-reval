#!/usr/bin/env python3
"""Build the P2-040 dusk and night perched songbird batch.
...
GLB embeds a compact feather albedo, normal, and roughness set. The thrush's
speckled breast and yellowhammer's yellow breast are separate geometry regions so
species remain distinguishable at gameplay scale.

Run from the repository root:
    blender --background --factory-startup --python \
        generated/comfyui/bird_songbird_dusk_v1/generate_songbirds.py
"""
...
EVIDENCE_DIR = ROOT / "generated" / "comfyui" / "bird_songbird_dusk_v1"
PREVIEW_DIR = EVIDENCE_DIR / "previews"
BLENDER_VERSION = "Blender 5.2 LTS"
GENERATOR_VERSION = "songbird_dusk_v1"
TEXTURE_SIZE = 128
...
SPECIES: dict[str, dict[str, object]] = {
    "common_blackbird": {
        "scale_m": 0.25,
        "body": (0.150, 0.082, 0.078),
        "head_radius": 0.040,
        "tail": 0.070,
        "wing": (0.035, 0.064, 0.050),
        "beak": 0.028,
        "leg": 0.042,
        "body_color": (0.055, 0.060, 0.058),
        "wing_color": (0.025, 0.030, 0.029),
        "head_color": (0.035, 0.040, 0.038),
        "breast_color": (0.065, 0.070, 0.066),
        "beak_color": (0.72, 0.47, 0.12),
        "eye_color": (0.006, 0.004, 0.002),
        "eye_ring_color": (0.82, 0.63, 0.20),
    },
    "song_thrush": {
        "scale_m": 0.23,
        "body": (0.145, 0.082, 0.078),
        "head_radius": 0.038,
        "tail": 0.060,
        "wing": (0.034, 0.061, 0.049),
        "beak": 0.025,
        "leg": 0.040,
        "body_color": (0.50, 0.37, 0.24),
        "wing_color": (0.28, 0.23, 0.17),
        "head_color": (0.46, 0.34, 0.23),
        "breast_color": (0.72, 0.61, 0.43),
        "beak_color": (0.50, 0.36, 0.18),
        "eye_color": (0.010, 0.007, 0.003),
        "speckles": True,
    },
    "common_nightingale": {
        "scale_m": 0.16,
        "body": (0.102, 0.056, 0.060),
        "head_radius": 0.028,
        "tail": 0.052,
        "wing": (0.023, 0.043, 0.035),
        "beak": 0.017,
        "leg": 0.028,
        "body_color": (0.43, 0.32, 0.22),
        "wing_color": (0.29, 0.23, 0.17),
        "head_color": (0.38, 0.29, 0.20),
        "breast_color": (0.60, 0.48, 0.32),
        "beak_color": (0.43, 0.30, 0.16),
        "eye_color": (0.010, 0.007, 0.003),
    },
    "yellowhammer": {
        "scale_m": 0.17,
        "body": (0.108, 0.058, 0.062),
        "head_radius": 0.029,
        "tail": 0.050,
        "wing": (0.024, 0.045, 0.037),
        "beak": 0.018,
        "leg": 0.029,
        "body_color": (0.68, 0.60, 0.18),
        "wing_color": (0.27, 0.25, 0.16),
        "head_color": (0.18, 0.16, 0.09),
        "breast_color": (0.86, 0.74, 0.20),
        "beak_color": (0.26, 0.22, 0.12),
        "eye_color": (0.008, 0.006, 0.002),
        "yellow_breast": True,
    },
}
...
def _uv(obj: bpy.types.Object) -> None:
    ...
    obj.select_set(False)
    atlas_roles = ("body", "wing", "head", "breast", "cheek", "beak", "leg", "eye", "tail", "bib", "speckle")
...
def _build_bird(species: str, spec: dict[str, object]) -> list[bpy.types.Object]:
    ...
    # Species-specific breast patches are shallow solids so the markings survive
    # glTF import and remain legible at the small gameplay-camera scale.
    if spec.get("speckles"):
        objects.append(_ellipsoid("SpeckledBreast", (0.0, -body_len * 0.49, body_z + body_height * 0.03), (body_width * 0.43, body_len * 0.08, body_height * 0.42), mats["breast"], segments=12, rings=7))
        for speckle_index, (speckle_x, speckle_z, speckle_scale) in enumerate(
            ((-0.020, 0.010, 1.0), (0.012, 0.019, 0.82), (-0.006, -0.006, 0.72), (0.022, -0.015, 0.62), (-0.024, -0.022, 0.58))
        ):
            objects.append(_ellipsoid(f"BreastSpeckle_{speckle_index}", (speckle_x, -body_len * 0.575, body_z + body_height * 0.03 + speckle_z), (body_width * 0.075 * speckle_scale, body_len * 0.012, body_height * 0.065 * speckle_scale), mats["speckle"], segments=8, rings=5))
    elif spec.get("yellow_breast"):
    ...
    # Two slightly separated tail vanes read as a short songbird tail.
    for side in (-1.0, 1.0):
        tail = _ellipsoid(
    ...
    # Eyes and the blackbird's warm eye ring remain geometry for reliable import.
    for side in (-1.0, 1.0):
        eye = _ellipsoid(
        ...
        objects.append(eye)
        if species == "common_blackbird":
            objects.append(_ellipsoid(f"EyeRing_{'L' if side < 0 else 'R'}", (side * head_r * 0.52, head_y - head_r * 0.775, head_z + head_r * 0.10), (head_r * 0.19, head_r * 0.035, head_r * 0.19), mats["breast"], segments=8, rings=5))

    # Yellowhammer's dark head cap reinforces its yellow-body silhouette.
    if species == "yellowhammer":
        objects.append(_ellipsoid("DarkHeadCap", (0.0, head_y - head_r * 0.02, head_z + head_r * 0.48), (head_r * 0.82, head_r * 0.62, head_r * 0.34), mats["bib"], segments=10, rings=5))

    # Legs and three short forward toes per foot keep the perched pose grounded.
    for side in (-1.0, 1.0):
...
def main() -> int:
    ...
    if {entry["species"] for entry in report} != set(SPECIES):
        raise RuntimeError("report species set does not match the P2-040 manifest")
    state = {
        "asset_id": "assets.birds.songbird_dusk_batch",
        "task": "P2-040",
