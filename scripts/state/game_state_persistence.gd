...
static func save_payload(state: GameState) -> Dictionary:
	...
	return {
		...
		"magic_resources": _int_dictionary(state._magic_resources),
		"magic_grants": _bool_dictionary(state._magic_grants),
		"forge_conduit_bound": state.is_forge_conduit_bound(),
...
static func load_payload(state: GameState, payload: Dictionary) -> Array[String]:
	...
	state._magic_resources = _load_magic_resources(candidate.get("magic_resources", {}), errors)
	state._magic_grants = _load_bool_dictionary(
		candidate.get("magic_grants", {}), errors, "magic_grants"
	)
...
static func _load_magic_resources(
	source: Variant, errors: Array[String]
) -> Dictionary[StringName, int]:
	var out := _load_int_dictionary(source, errors, "magic_resources")
	for resource_id in [GameState.MAGIC_RESOURCE_WILLPOWER, GameState.MAGIC_RESOURCE_PIETY]:
		out[resource_id] = maxi(0, int(out.get(resource_id, GameState.MAGIC_RESOURCE_DEFAULT)))
	return out
