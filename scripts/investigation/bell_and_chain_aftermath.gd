class_name BellAndChainAftermath
extends Node

## Patrol bark differences after the Viru Gate chain install (P4-002).

const MODEL := preload("res://scripts/investigation/bell_and_chain_aftermath_model.gd")
...
func commit_install_for_test() -> bool:
	if _night_consequence != null:
		return _night_consequence.commit_install_for_test()
...
func _spawn_patrol_bark(
	...
	_patrol_bark.name = "BellAndChainPatrolBark"
