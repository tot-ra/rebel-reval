class_name PriceOfANameInstallConsequence
extends Node

## Coastal Gate detention install encounter for quest.price_of_a_name (P4-006).
...
const INTERACTABLE_ID := &"interact.price_of_a_name.detention_install"
...
const RECORD_SUBTLE := &"forged.price_of_a_name.subtle_defect"
const RECORD_SECRET := &"forged.price_of_a_name.secret_feature"
...
func commit_install_for_test() -> bool:
	return _commit_detention_install()
...
func _on_encounter_resolved() -> void:
	...
	_set_outcome_ui_visible(false)
	_teardown_combat_actors()
	_commit_detention_install()


func _commit_detention_install() -> bool:
...
func _spawn_combat_actors() -> void:
	if _actors == null or _definition == null:
		return
	_teardown_combat_actors()
	var watch_pos := MapVerification.anchor_position(_definition, INSTALL_ANCHOR)
	var sarge_pos := MapVerification.anchor_position(_definition, SERGEANT_ANCHOR)
	watchman = _make_enemy(
		"PriceOfANameWatchman", watch_pos, EnemyArchetype.watchman(), Color(0.62, 0.58, 0.34, 1.0)
	)
	sergeant = _make_enemy(
		"PriceOfANameSergeant", sarge_pos, EnemyArchetype.sergeant(), Color(0.55, 0.28, 0.42, 1.0)
	)
...
func _build_install_interactable() -> void:
	...
	_install_interactable.prompt = "Install the detention shackles and name-plate"
...
func _build_outcome_ui() -> void:
	...
	_actions_layer.name = "PriceOfANameInstallOutcomeActions"
