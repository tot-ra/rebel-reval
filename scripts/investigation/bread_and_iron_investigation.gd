class_name BreadAndIronInvestigation
extends Node

## Daytime civic market evidence loop for quest.bread_and_iron (P4-004).
...
const SITES: Array[Dictionary] = [
	{
		"interactable_id": &"interact.bread_and_iron.weighbridge",
		"position_kind": &"prop",
		"position_id": &"cistern",
		"dialogue_id": &"dialogue.bread_and_iron.inspect_weighbridge",
		"fact_id": &"fact.bread_and_iron.short_weight_tally",
		"prompt": "Inspect the civic weighbridge",
	},
	{
		"interactable_id": &"interact.bread_and_iron.grain_lane",
		"position_kind": &"prop",
		"position_id": &"market_stall_turg_north",
		"dialogue_id": &"dialogue.bread_and_iron.inspect_grain_lane",
		"fact_id": &"fact.bread_and_iron.diverted_grain_sacks",
		"prompt": "Inspect the Vana Turg grain lane",
	},
	{
		"interactable_id": &"interact.bread_and_iron.supplier_ledgers",
		"position_kind": &"prop",
		"position_id": &"merchants_house",
		"dialogue_id": &"dialogue.bread_and_iron.inspect_supplier_ledgers",
		"fact_id": &"fact.bread_and_iron.supplier_ledger_fraud",
		"prompt": "Read the market weighbridge ledgers",
	},
	{
		"interactable_id": &"interact.bread_and_iron.raide_stall",
		"position_kind": &"prop",
		"position_id": &"market_stall_turg_south",
		"dialogue_id": &"dialogue.bread_and_iron.inspect_raide_stall",
		"fact_id": &"fact.bread_and_iron.raide_empty_bins",
		"prompt": "Inspect Mare Raide's grain stall",
	},
]
...
var _supplier_stock_granted := false
...
func _build_dialogue_stack() -> void:
	...
	_dialogue_ui.name = "BreadAndIronDialogueUI"
	_scene_root.add_child(_dialogue_ui)

	_runner = RunnerScript.new()
	_runner.name = "BreadAndIronDialogueRunner"
	add_child(_runner)
...
func _build_view_binder(view_runtime: MapViewRuntime, definition: MapDefinition) -> void:
	...
	_view_binder.name = "BreadAndIronInteractableViewBinder"
...
func _grant_supplier_stock() -> void:
	if _supplier_stock_granted or SessionState.state == null:
		return
	if SessionState.state.has_item(ModelScript.SUPPLIER_ITEM_ID):
		_supplier_stock_granted = true
		return
	SessionState.state.add_item(ModelScript.SUPPLIER_ITEM_ID)
	_supplier_stock_granted = true
