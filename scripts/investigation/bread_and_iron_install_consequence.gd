class_name BreadAndIronInstallConsequence
extends Node

## Civic weighing-house bracket install encounter for quest.bread_and_iron (P4-004).
...
func _on_encounter_resolved() -> void:
	...
	_set_outcome_ui_visible(false)
	_teardown_combat_actors()
	_commit_scale_install()


func _commit_scale_install() -> bool:
...
func _spawn_combat_actors() -> void:
	...
	watchman = _make_enemy(
		"BreadAndIronWatchman", watch_pos, EnemyArchetype.watchman(), Color(0.62, 0.58, 0.34, 1.0)
	)
	sergeant = _make_enemy(
		"BreadAndIronSergeant", sarge_pos, EnemyArchetype.sergeant(), Color(0.55, 0.28, 0.42, 1.0)
	)
...
func _build_install_interactable() -> void:
	...
	_install_interactable.prompt = "Install the weighing-house pivot brackets"
...
func _build_outcome_ui() -> void:
	...
	_actions_layer.name = "BreadAndIronInstallOutcomeActions"
