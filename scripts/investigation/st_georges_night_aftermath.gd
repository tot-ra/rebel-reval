class_name StGeorgesNightAftermath
extends Node

## Patrol bark differences after the St. George's Night act-boundary choice (P4-008).

const MODEL := preload("res://scripts/investigation/st_georges_night_aftermath_model.gd")
...
func setup(
	...
	if _climax != null and _climax.has_signal("choice_committed"):
		_climax.choice_committed.connect(_on_choice_committed)
...
func _on_choice_committed(_choice: StringName) -> void:
	_sync_aftermath()
