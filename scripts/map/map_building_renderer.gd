...
static func create_building(
	...
	match kind:
		...
		_:
			_draw_house(
				visuals, local_footprint, wall_height, target, time_of_day, String(building["id"])
			)
	return body
...
static func _draw_house(
	...
	_add_polygon(
		parent,
		"GroundShadow",
		_rect_points(shadow_rect),
		Color(ink, MapVisualStyle.shadow_alpha(target, time_of_day)),
		-3
	)
	...
	var facade := Rect2(
		footprint.position.x, footprint.end.y - facade_height, footprint.size.x, facade_height
	)
	...
	_add_polygon(
		parent, "NorthFacade", _rect_points(north_face), plaster.lightened(0.08), 0, ink, outline
	)
	...
	for index in timber_count:
		...
		_add_polygon(
			parent,
			"Timber%d" % index,
			_rect_points(Rect2(x - 2.5, facade.position.y, 5.0, facade.size.y)),
			timber,
			3
		)
	_add_polygon(
		...
		"CrossBeam",
		_rect_points(
			Rect2(facade.position.x, facade.position.y + facade.size.y * 0.42, facade.size.x, 4.0)
		),
		timber,
		3
	)
	...
	var door := Rect2(
		facade.get_center().x - door_width * 0.5, facade.end.y - 25.0, door_width, 25.0
	)
	...
	for window_index in 2:
		...
		var window_rect := Rect2(
			facade.get_center().x + side * footprint.size.x * 0.27 - 6.0,
			facade.position.y + 6.0,
			12.0,
			10.0
		)
		_add_polygon(
			parent, "Window%d" % window_index, _rect_points(window_rect), window, 4, ink, outline
		)

	var roof_base := Rect2(
		footprint.position.x - 9.0,
		footprint.position.y - wall_height * 0.44,
		footprint.size.x + 18.0,
		footprint.size.y * 0.58
	)
	...
	if target == MapVisualStyle.TARGET_WOODCUT:
		...
		for index in mini(8, int(roof_base.size.x / 18.0)):
			_add_polygon(
				...
				_rect_points(
					Rect2(
						roof_base.position.x + 8.0 + index * 18.0, roof_base.end.y - 8.0, 9.0, 4.0
					)
				),
				roof.lightened(0.13),
				7
		...
		_add_polygon(
			parent,
			"PaintedWash",
			_rect_points(Rect2(facade.position + Vector2(5, 5), facade.size - Vector2(10, 10))),
			wash,
			2
		)
	...
	if abs(building_id.hash()) % 2 == 0:
		_add_polygon(
			parent,
			"ForgeSign",
			_rect_points(Rect2(door.end.x + 5.0, facade.position.y + 7.0, 8.0, 8.0)),
...
static func _draw_interior_block(
	...
	_add_polygon(
		parent, "BlockMass", _rect_points(footprint), timber.darkened(0.12), 0, ink, outline
	)
	var face := Rect2(
		footprint.position.x,
		footprint.end.y - minf(footprint.size.y * 0.55, wall_height * 0.35),
		footprint.size.x,
		minf(footprint.size.y * 0.55, wall_height * 0.35)
	)
...
static func _draw_wall(
	...
	_add_polygon(
		parent, "WallMass", _rect_points(footprint), stone.darkened(0.18), -1, ink, outline
	)
	var face_height := maxf(10.0, minf(footprint.size.y * 0.65, wall_height * 0.45))
	var face := Rect2(
		footprint.position.x, footprint.end.y - face_height, footprint.size.x, face_height
	)
	_add_polygon(parent, "SouthFace", _rect_points(face), stone, 1, ink, outline)
	var coping := Rect2(
		footprint.position.x - 2.0, footprint.position.y - 6.0, footprint.size.x + 4.0, 7.0
	)
...
static func _add_hatching(parent: Node, rect: Rect2, color: Color, layer: int) -> void:
	for index in maxi(1, int(rect.size.x / 18.0)):
		...
		line.points = PackedVector2Array(
			[Vector2(x, rect.end.y - 4.0), Vector2(x + 10.0, rect.position.y + 5.0)]
		)
		line.width = 1.0
