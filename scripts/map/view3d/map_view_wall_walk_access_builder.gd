...
static func add_to(
	root: Node3D, prop: Dictionary, cell_size: int, definition: MapDefinition
) -> void:
	...
	var width := minf(
		size.y if along_x else size.x, MapViewMeshBuilderConfig.WALL_WALK_ACCESS_STAIR_WIDTH
	)
	for step_index in step_count:
		var progress := (float(step_index) + 0.5) / float(step_count)
		var along := (
			-run * 0.5 + progress * run * MapViewMeshBuilderConfig.WALL_WALK_ACCESS_CLIMB_FRACTION
		)
		if (facing.x if along_x else facing.y) < 0.0:
			along = -along
		var center := (
			Vector3(along, target * progress, 0.0)
			if along_x
			else Vector3(0.0, target * progress, along)
		)
		var tread_size := (
			Vector3(tread + 0.03, MapViewMeshBuilderConfig.WALL_WALK_ACCESS_TREAD_THICKNESS, width)
			if along_x
			else Vector3(
	...
	var landing_size := (
		Vector3(landing_length, 0.14, width) if along_x else Vector3(width, 0.14, landing_length)
	)
	...
	var finish := (
		start
		+ direction * run * MapViewMeshBuilderConfig.WALL_WALK_ACCESS_CLIMB_FRACTION
		+ Vector3.UP * target
	)
	for side in [-1.0, 1.0]:
		...
		_add_access_beam(
			root,
			"WallStairRail%d" % int(side),
			start + offset * side + Vector3.UP * 0.55,
			(
				finish
				+ offset * side
				+ Vector3.UP * MapViewMeshBuilderConfig.WALL_WALK_ACCESS_RAIL_HEIGHT
			),
			0.09
		)
		for progress in [0.0, 0.25, 0.5, 0.75, 1.0]:
			...
			_add_access_beam(
				root,
				"WallStairPost%d_%d" % [int(side), int(progress * 100.0)],
				foot,
				foot + Vector3.UP * MapViewMeshBuilderConfig.WALL_WALK_ACCESS_RAIL_HEIGHT,
				0.08
			)


static func _add_access_beam(
