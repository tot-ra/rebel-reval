...
static func build_interior_shell(definition: MapDefinition) -> Node3D:
	...
	var ceiling_plane := (
		wall_height + MapViewMeshBuilderConfig.INTERIOR_CEILING_FIRST_PERSON_HEADROOM
	)
	...
	ceiling.material_override = MapViewMaterials.wall_surface_for_building(
		&"interior_ceiling", &"plank", MapViewMeshBuilderConfig.INTERIOR_CEILING_COLOR, ceiling_size
	)
...
static func _add_exposed_beams(root: Node3D, floor_bounds: Rect2, ceiling_plane: float) -> void:
	...
	var primary_count := maxi(
		2, int(primary_span / MapViewMeshBuilderConfig.INTERIOR_BEAM_SPACING) + 1
	)
	var secondary_count := maxi(
		2, int(secondary_span / (MapViewMeshBuilderConfig.INTERIOR_BEAM_SPACING * 1.35)) + 1
	)
	...
	for index in primary_count:
		...
		MapViewMeshBuilderPrimitives.box(
			root, "PrimaryBeam%d" % index, beam_size, position, &"timber"
		)
	...
	for index in range(1, secondary_count - 1):
		...
		var beam_size := Vector3(
			...
			(
				floor_bounds.size.y * 0.92
				if along_x
				else MapViewMeshBuilderConfig.INTERIOR_BEAM_THICKNESS
			)
		)
		if not along_x:
		...
		MapViewMeshBuilderPrimitives.box(
			root, "SecondaryBeam%d" % index, beam_size, position, &"timber"
		)
