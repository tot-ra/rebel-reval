...
static func _build_standing(
	surface: SurfaceTool, profile: Dictionary, archetype: StringName
) -> void:
	...
	for stem_index in stem_count:
		var yaw := (
			...
			+ MeshMath.hash01(stem_index, 17, 2903) * 0.62
		)
		...
		if (
			archetype
			in [
				PlantSpecies.ARCHETYPE_HERB,
				PlantSpecies.ARCHETYPE_STALK,
			...
			for leaf_index in leaf_pairs:
				...
				var leaf_length := (
					spread * lerpf(0.55, 0.9, MeshMath.hash01(stem_index, leaf_index, 2953))
				)
		...
		match archetype:
			...
			PlantSpecies.ARCHETYPE_REED:
				_append_seed_head(
					...
					side,
					stem_height * 0.13,
					maxf(0.018, spread * 0.07),
					accent,
					stem_index
				)
				_append_leaf(
					surface,
					root + Vector3.UP * stem_height * 0.16,
					radial,
					spread * 0.75,
					spread * 0.08,
					leaf_color
			...
			PlantSpecies.ARCHETYPE_CATTAIL:
				_append_cattail_head(
					surface,
					top - Vector3.UP * stem_height * 0.10,
					side,
					stem_height * 0.17,
					maxf(0.025, spread * 0.09),
					accent
				)
				_append_leaf(
					surface,
					root + Vector3.UP * stem_height * 0.12,
					radial,
					spread * 0.8,
					spread * 0.07,
					leaf_color
			...
			PlantSpecies.ARCHETYPE_VINE:
				_append_flower(
					surface,
					top - Vector3.UP * stem_height * 0.08,
					maxf(0.035, spread * 0.10),
					accent,
					stem_index
				)
...
static func _build_rosette(surface: SurfaceTool, profile: Dictionary, broad: bool) -> void:
	...
	for leaf_index in count:
		var yaw := (
			TAU * float(leaf_index) / float(count) + MeshMath.hash01(leaf_index, 37, 3001) * 0.32
		)
		...
		_append_leaf(
			surface,
			center,
			(direction + Vector3.UP * lift / maxf(leaf_length, 0.01)).normalized(),
			leaf_length,
			width,
			color.lightened(float(leaf_index % 3) * 0.035)
		)
	if broad:
		var accent: Color = profile["accent"]
...
static func _build_moss(surface: SurfaceTool, profile: Dictionary) -> void:
	...
	for mound_index in count:
		...
		_append_low_crown(
			surface, center, radius, height * 0.28, color.lightened(float(mound_index % 3) * 0.04)
		)
...
static func _build_aquatic(surface: SurfaceTool, profile: Dictionary) -> void:
	...
	for pad_index in count:
		...
		var center := (
			Vector3(cos(yaw), 0.018 + float(pad_index % 2) * 0.006, sin(yaw)) * spread * 0.28
		)
		_append_horizontal_fan(
			surface,
			center,
			spread * lerpf(0.16, 0.25, MeshMath.hash01(pad_index, 59, 3061)),
			color.lightened(float(pad_index % 3) * 0.035),
			8
		)
	var accent: Color = profile["accent"]
...
static func _append_leaf(
	...
	_append_colored_triangle(
		surface, root, tip, left, color, Vector2(0.5, 0.0), Vector2(0.5, 1.0), Vector2(0.0, 0.48)
	)


static func _append_flower(
	surface: SurfaceTool, center: Vector3, radius: float, color: Color, seed: int
) -> void:
	var petal_count := 6
	for petal_index in petal_count:
		var yaw := (
			TAU * float(petal_index) / float(petal_count)
			+ MeshMath.hash01(petal_index, seed, 3083) * 0.12
		)
	...
	_append_low_crown(
		surface,
		center + Vector3.UP * radius * 0.08,
		radius * 0.20,
		radius * 0.16,
		Color(0.92, 0.66, 0.12)
	)


static func _append_seed_head(
	surface: SurfaceTool,
	bottom: Vector3,
	side: Vector3,
	height: float,
	...
	for grain_index in 4:
		...
		_append_leaf(
			surface,
			center,
			(grain_side + Vector3.UP * 0.18).normalized(),
			height * lerpf(0.18, 0.28, MeshMath.hash01(grain_index, seed, 3109)),
			width * 1.5,
			color.lightened(0.04)
		)


static func _append_cattail_head(
	surface: SurfaceTool, bottom: Vector3, side: Vector3, height: float, width: float, color: Color
) -> void:
...
static func _append_low_crown(
	surface: SurfaceTool, center: Vector3, radius: float, height: float, color: Color
) -> void:
	...
	for side_index in 8:
		...
		_append_colored_triangle(
			surface, a, b, top, color, Vector2(0.0, 0.0), Vector2(1.0, 0.0), Vector2(0.5, 1.0)
		)


static func _append_horizontal_fan(
	surface: SurfaceTool, center: Vector3, radius: float, color: Color, sides: int
) -> void:
	for side_index in sides:
		...
		var b := center + Vector3(cos(b_angle) * radius, 0.0, sin(b_angle) * radius)
		_append_colored_triangle(
			surface, center, a, b, color, Vector2(0.5, 0.5), Vector2(0.0, 0.0), Vector2(1.0, 0.0)
		)
