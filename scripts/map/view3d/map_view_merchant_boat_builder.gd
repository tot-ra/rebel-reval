...
static func add_to(root: Node3D, faction_id: StringName = FactionHeraldry.HANSEATIC) -> void:
	...
	MapViewMeshBuilderPrimitives.box(
		root, "Rudder", Vector3(0.12, 0.9, 0.52), Vector3(-3.4, 0.32, 0.0), &"timber"
	)
	_add_spar(
		root, "Tiller", Vector3(-3.34, 0.73, 0.0), Vector3(-2.45, 1.32, 0.0), 0.045, &"timber"
	)


static func _add_cargo(root: Node3D) -> void:
	MapViewMeshBuilderPrimitives.box(
		root, "CargoHatch", Vector3(1.35, 0.1, 1.05), Vector3(0.72, 1.08, 0.0), &"ink"
	)
	for side in [-1.0, 1.0]:
		var side_name := "Port" if side < 0.0 else "Starboard"
		_add_spar(
			root,
			"HatchCoaming%s" % side_name,
			Vector3(0.02, 1.16, side * 0.59),
			Vector3(1.42, 1.16, side * 0.59),
			0.045,
			&"timber"
		)
	_add_spar(
	...
	_add_spar(
		root,
		"HatchCoamingFore",
		Vector3(1.42, 1.16, -0.59),
		Vector3(1.42, 1.16, 0.59),
		0.045,
		&"timber"
	)
	MapViewMeshBuilderPrimitives.box(
		root, "CargoCrateLarge", Vector3(0.72, 0.62, 0.66), Vector3(-0.76, 1.34, -0.42), &"wood"
	)
	MapViewMeshBuilderPrimitives.box(
		root, "CargoCrateSmall", Vector3(0.52, 0.46, 0.5), Vector3(-0.84, 1.26, 0.42), &"timber"
	)
...
static func _add_rig(root: Node3D, faction_id: StringName = FactionHeraldry.HANSEATIC) -> void:
	...
	_add_spar(
		root, "Yard", Vector3(-0.14, 5.48, -2.18), Vector3(-0.14, 5.48, 2.18), 0.075, &"timber"
	)
	...
	_add_spar(
		rigging, "Forestay", Vector3(-0.18, 6.16, 0.0), Vector3(3.32, 1.28, 0.0), 0.018, &"ink"
	)
	_add_spar(
		rigging, "Backstay", Vector3(-0.18, 6.16, 0.0), Vector3(-3.08, 1.24, 0.0), 0.018, &"ink"
	)
	for side in [-1.0, 1.0]:
		var side_name := "Port" if side < 0.0 else "Starboard"
		_add_spar(
			rigging,
			"Shroud%sA" % side_name,
			Vector3(-0.18, 5.35, 0.0),
			Vector3(-0.82, 1.04, side * 1.34),
			0.018,
			&"ink"
		)
		_add_spar(
			rigging,
			"Shroud%sB" % side_name,
			Vector3(-0.18, 5.0, 0.0),
			Vector3(0.18, 1.02, side * 1.4),
			0.018,
			&"ink"
		)
		_add_spar(
			rigging,
			"YardLift%s" % side_name,
			Vector3(-0.18, 6.12, 0.0),
			Vector3(-0.14, 5.48, side * 2.18),
			0.014,
			&"ink"
		)
		_add_spar(
			rigging,
			"Sheet%s" % side_name,
			Vector3(0.1, 2.96, side * 1.62),
			Vector3(-2.22, 1.48, side * 0.86),
			0.014,
			&"ink"
		)
...
static func _add_hull_timber(root: Node3D) -> void:
	...
	_add_spar(
		root, "Sternpost", Vector3(-3.26, -0.34, 0.0), Vector3(-3.43, 1.24, 0.0), 0.07, &"timber"
	)
	_add_spar(
		root, "BowStem", Vector3(3.22, -0.26, 0.0), Vector3(3.51, 1.48, 0.0), 0.075, &"timber"
	)
	...
	for side in [-1.0, 1.0]:
		...
		for rail_index in rail_stations.size() - 1:
			...
			_add_spar(
				root, "ClinkerStrake%s%d" % [side_name, rail_index], start, end, 0.035, &"timber"
			)
		for rail_index in rail_stations.size() - 1:
			var start := rail_stations[rail_index]
			var end := rail_stations[rail_index + 1]
...
static func _add_castle(
	root: Node3D, node_name: String, aft_x: float, fore_x: float, deck_y: float, half_beam: float
) -> void:
	...
	for side in [-1.0, 1.0]:
		...
		for post_index in 3:
			...
			_add_spar(
				castle,
				"Post%s%d" % [side_name, post_index],
				Vector3(x, deck_y, side_z),
				Vector3(x, deck_y + post_height, side_z),
				0.035,
				&"timber"
			)
		_add_spar(
			castle,
			"Rail%s" % side_name,
			Vector3(aft_x, deck_y + post_height, side_z),
			Vector3(fore_x, deck_y + post_height, side_z),
			0.04,
			&"timber"
		)
	_add_spar(
		castle,
		"EndRail",
		Vector3(aft_x, deck_y + post_height, -half_beam * 0.82),
		Vector3(aft_x, deck_y + post_height, half_beam * 0.82),
		0.04,
		&"timber"
	)
	...
	for row in row_heights.size() - 1:
		for column in COLUMN_COUNT:
			...
			var vertices := [
				...
				_sail_point(
					row_heights[row + 1], row_half_widths[row + 1], row_bulges[row + 1], right_t
				),
				_sail_point(
					row_heights[row + 1], row_half_widths[row + 1], row_bulges[row + 1], left_t
				),
			]
			...
			for vertex_index in [0, 1, 2, 0, 2, 3]:
				...
				surface.set_uv(
					Vector2(
						left_t if vertex_index in [0, 3] else right_t,
						(
							float(row + (1 if vertex_index in [2, 3] else 0))
							/ float(row_heights.size() - 1)
						)
					)
				)
...
static func _sail_point(
	height: float, half_width: float, bulge: float, horizontal_t: float
) -> Vector3:
