...
## WS-13e / R-939: visible stone fill stays inside the crib, under CRIB_TOP_CLEARANCE.
## Three smaller irregular stones along each face (three height layers on a deep
## bank) still share the logs MeshInstance as a second surface.
const CRIB_RUBBLE_ALONG := 3
const CRIB_RUBBLE_MAX_LAYERS := 3
...
## entire district's vegetation micro-geometry.
...
const SCATTER_GRASS_VISIBILITY_RANGE := 45.0
const SCATTER_GRASS_VISIBILITY_RANGE_MARGIN := 8.0
