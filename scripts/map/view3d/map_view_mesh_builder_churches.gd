...
const ST_OLAF_ID := &"st_olaf_silhouette"
const ST_OLAF_RENDERER := &"st_olaf_1343"
const ST_OLAF_ROOF_PITCH := 0.76
const NAVE_ROOF_PITCH := 0.72
const BUTTRESS_COUNT := 4
const LANCET_COUNT := 5


static func is_st_olaf_church(building: Dictionary) -> bool:
	return StringName(String(building.get("id", ""))) == ST_OLAF_ID


static func build_st_olaf_church(building: Dictionary, cell_size: int) -> Node3D:
	var root := Node3D.new()
	...
	root.set_meta(&"church_renderer", ST_OLAF_RENDERER)
	root.set_meta(&"historical_phase", &"compact_1343_mass")
	root.set_meta(&"excluded_features", ["15th_century_chancel", "later_basilica", "giant_spire"])
	...
	wall_mesh.size = Vector3(size.x, height, size.y * 0.82)
	walls.mesh = wall_mesh
	walls.position = Vector3(0.0, height * 0.5, size.y * 0.09)
	walls.material_override = MapViewMaterials.wall_surface_for_building(
		ST_OLAF_ID, &"limestone", wall_color, wall_mesh.size
	)
	...
	nave_roof.mesh = MapViewMeshBuilderPrimitives.gabled_roof_mesh(
		Vector2(size.x, size.y * 0.82), true, 0.22, true, ST_OLAF_ROOF_PITCH
	)
	nave_roof.position = Vector3(0.0, height, size.y * 0.09)
	var roof_color := Color(
		building.get("roof_color", MapViewMeshBuilderConfig.DEFAULT_ROOF_COLOR)
	)
	nave_roof.material_override = MapViewMaterials.roof_surface_for_building(
		ST_OLAF_ID, &"tile", roof_color
	)
	root.add_child(nave_roof)

	# WHY: The Spring-1343 silhouette is a compact nave with completed vault work,
	# not the later basilica/chancel or tourist-era spire. A blunt west tower gives
	# the church its landmark read without inventing a later vertical skyline.
	_add_st_olaf_west_tower(root, size, height, wall_color)
	_add_st_olaf_vault_buttresses(root, size, height)
	_add_st_olaf_lancets(root, size, height)
	var lights: BuildingWindowLights3D = MapViewMeshBuilderConfig.WINDOW_LIGHTS_SCRIPT.new()
	root.add_child(lights)
	lights.configure(ST_OLAF_ID)
	return root


static func _add_st_olaf_west_tower(
	root: Node3D, size: Vector2, height: float, wall_color: Color
) -> void:
	var tower := Node3D.new()
	tower.name = "WestTower"
	root.add_child(tower)
	var tower_width := minf(3.6, size.y * 0.52)
	var tower_depth := minf(3.8, size.y * 0.86)
	var tower_height := height * 1.42
	var tower_x := -size.x * 0.5 + tower_width * 0.5
	var masonry := MeshInstance3D.new()
	masonry.name = "Masonry"
	var masonry_mesh := BoxMesh.new()
	masonry_mesh.size = Vector3(tower_width, tower_height, tower_depth)
	masonry.mesh = masonry_mesh
	masonry.position = Vector3(tower_x, tower_height * 0.5, size.y * 0.09)
	masonry.material_override = MapViewMaterials.wall_surface_for_building(
		ST_OLAF_ID, &"limestone", wall_color.lightened(0.03), masonry_mesh.size
	)
	...
	for side: float in [-1.0, 1.0]:
		MapViewMeshBuilderPrimitives.box(
			...
			Vector3(tower_width * 0.72, 0.16, 0.12),
			Vector3(tower_x, tower_height * 0.76, size.y * 0.09 + side * tower_depth * 0.46),
			&"window"
		)
	var cap := MeshInstance3D.new()
	cap.name = "TowerCoping"
	var cap_mesh := BoxMesh.new()
	cap_mesh.size = Vector3(tower_width + 0.18, 0.18, tower_depth + 0.18)
	cap.mesh = cap_mesh
	cap.position = Vector3(tower_x, tower_height + 0.09, size.y * 0.09)
	cap.material_override = MapViewMaterials.role_for_size(&"stone", cap_mesh.size)
	tower.add_child(cap)


static func _add_st_olaf_vault_buttresses(root: Node3D, size: Vector2, height: float) -> void:
	var span := size.x - 3.2
	for index in BUTTRESS_COUNT:
		...
		for side: float in [-1.0, 1.0]:
			MapViewMeshBuilderPrimitives.box(
				root,
				"VaultButtress_%s_%02d" % ["N" if side < 0.0 else "S", index],
				Vector3(0.34, height * 0.68, 0.42),
				Vector3(along, height * 0.34, side * (size.y * 0.41 + 0.18)),
				&"stone"
			)


static func _add_st_olaf_lancets(root: Node3D, size: Vector2, height: float) -> void:
	var run := size.x - 3.0
	var face_z := size.y * 0.41 + 0.055
	var window_height := minf(1.42, height * 0.3)
	...
	for index in LANCET_COUNT:
		...
		for side: float in [-1.0, 1.0]:
			MapViewMeshBuilderPrimitives.box(
				root,
				"VaultLancet_%s_%02d" % ["N" if side < 0.0 else "S", index],
				Vector3(0.28, window_height, 0.08),
