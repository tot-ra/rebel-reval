...
static func create(spec: Dictionary) -> MapDefinition:
	...
	definition.seed = int(
		spec.get("seed", MapTypes.DEFAULT_SEED + abs(String(spec["map_id"]).hash()) % 10000)
	)
	...
	definition.camera_bounds = definition.cell_rect_to_world_rect(
		Rect2i(Vector2i.ZERO, definition.size_cells)
	)
...
static func structure(
	id: StringName, primitive: StringName, rect: Rect2i, height: float = 56.0
) -> Dictionary:
