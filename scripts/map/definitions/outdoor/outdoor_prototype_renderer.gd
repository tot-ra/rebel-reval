...
func _draw_terrain() -> void:
	...
	for y in grid.size_cells.y:
		for x in grid.size_cells.x:
			...
			if (
				terrain in [MapTypes.TERRAIN_SHALLOW_WATER, MapTypes.TERRAIN_DEEP_WATER]
				and hash % 3 == 0
			):
				draw_line(
					rect.position + Vector2(5, 12),
					rect.position + Vector2(27, 12),
...
func _draw_structure(building: Dictionary) -> void:
	...
	var wall_like := (
		primitive in [&"wall", &"palisade", &"pier", &"bridge", &"ditch_edge", &"tree_line"]
	)
	...
	if primitive in [&"gatehouse", &"stone_keep", &"coastal_bishop_castle", &"limestone_tower"]:
		var door := Rect2(
			rect.get_center() + Vector2(-10, 2), Vector2(20, rect.end.y - rect.get_center().y - 2)
		)
	...
	match primitive:
		...
		&"fishing_boat", &"merchant_boat":
			var footprint: Rect2 = prop.get(
				"footprint", Rect2(point - Vector2(16, 48), Vector2(32, 96))
			)
			var merchant_scale := 1.35 if primitive == &"merchant_boat" else 1.0
			var half_length := (
				maxf(36.0, maxf(footprint.size.x, footprint.size.y) * 0.42) * merchant_scale
			)
			var half_beam := (
				maxf(12.0, minf(footprint.size.x, footprint.size.y) * 0.3) * merchant_scale
			)
...
func _structure_color(primitive: StringName) -> Color:
	if (
		primitive
		in [&"wall", &"gatehouse", &"stone_keep", &"stone_hall", &"stone_church", &"monastic_range"]
	):
