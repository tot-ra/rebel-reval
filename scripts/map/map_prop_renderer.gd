...
static func create_prop(
	...
	shadow.polygon = Draw.ellipse(
		Vector2(0, 2) + MapVisualStyle.shadow_offset(target) * 0.35, Vector2(26, 9), 16
	)
	shadow.color = Color(
		MapVisualStyle.role_color(&"ink", target, time_of_day),
		MapVisualStyle.shadow_alpha(target, time_of_day)
	)
	shadow.z_index = -2
	...
	match prop["kind"] as StringName:
		MapTypes.PROP_KIND_ANVIL:
			Industrial.draw_anvil(root, target, time_of_day)
		MapTypes.PROP_KIND_HAY_STACK:
			Industrial.draw_hay_stack(root, prop, target, time_of_day)
		MapTypes.PROP_KIND_CART:
			Industrial.draw_cart(root, target, time_of_day)
		MapTypes.PROP_KIND_WELL:
			Industrial.draw_well(root, target, time_of_day)
		MapTypes.PROP_KIND_BARRELS:
			Harbor.draw_barrels(root, target, time_of_day)
		MapTypes.PROP_KIND_FURNACE:
			Industrial.draw_furnace(root, target, time_of_day)
		MapTypes.PROP_KIND_BELLOWS:
			Industrial.draw_bellows(root, target, time_of_day)
		MapTypes.PROP_KIND_BLACKSMITH_TONGS:
			Industrial.draw_blacksmith_tongs(root, target, time_of_day)
		MapTypes.PROP_KIND_BLACKSMITH_HAMMER:
			Industrial.draw_blacksmith_hammer(root, target, time_of_day)
		MapTypes.PROP_KIND_BLACKSMITH_PUNCH:
			Industrial.draw_blacksmith_punch(root, target, time_of_day)
		MapTypes.PROP_KIND_LEDGER:
			Industrial.draw_ledger(root, target, time_of_day)
		MapTypes.PROP_KIND_BED:
			Industrial.draw_bed(root, target, time_of_day)
		MapTypes.PROP_KIND_CHEST:
			Industrial.draw_chest(root, target, time_of_day)
		MapTypes.PROP_KIND_TABLE:
			Industrial.draw_table(root, target, time_of_day)
		MapTypes.PROP_KIND_SHELF:
			Industrial.draw_shelf(root, target, time_of_day)
		MapTypes.PROP_KIND_QUENCH:
			Industrial.draw_quench(root, target, time_of_day)
		MapTypes.PROP_KIND_STAIRS:
			Industrial.draw_stairs(root, target, time_of_day)
		MapTypes.PROP_KIND_STALL:
			Industrial.draw_stall(root, target, time_of_day)
		MapTypes.PROP_KIND_HEARTH:
			Industrial.draw_hearth(root, target, time_of_day)
		MapTypes.PROP_KIND_KITCHENWARE:
			Industrial.draw_kitchenware(root, prop, target, time_of_day)
		MapTypes.PROP_KIND_HOUSEHOLD_CLUTTER:
			Industrial.draw_household_clutter(root, prop, target, time_of_day)
		MapTypes.PROP_KIND_CHAIR:
			Industrial.draw_chair(root, target, time_of_day)
		MapTypes.PROP_KIND_CANDLE:
			Industrial.draw_candle(root, target, time_of_day)
		MapTypes.PROP_KIND_BUSH:
			Nature.draw_bush(root, target, time_of_day)
		MapTypes.PROP_KIND_TREE:
			Nature.draw_tree(root, prop, target, time_of_day)
		MapTypes.PROP_KIND_FISHING_BOAT:
			Harbor.draw_fishing_boat(root, target, time_of_day)
		MapTypes.PROP_KIND_MERCHANT_BOAT:
			Harbor.draw_merchant_boat(root, target, time_of_day)
		MapTypes.PROP_KIND_CARGO_CRATES:
			Harbor.draw_cargo_crates(root, target, time_of_day)
		MapTypes.PROP_KIND_TRADE_GOODS:
			Harbor.draw_trade_goods(root, target, time_of_day)
		MapTypes.PROP_KIND_TIMBER_FENCE:
			Nature.draw_timber_fence(root, prop, target, time_of_day)
		MapTypes.PROP_KIND_CATTLE:
			Nature.draw_cattle(root, target, time_of_day)
		MapTypes.PROP_KIND_SHEEP:
			Nature.draw_sheep(root, target, time_of_day)
		MapTypes.PROP_KIND_HORSE:
			Nature.draw_horse(root, target, time_of_day)
		MapTypes.PROP_KIND_BANNER:
			Heraldry.draw_banner(root, prop, target, time_of_day)
		_:
