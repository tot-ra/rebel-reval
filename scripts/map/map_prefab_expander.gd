...
static func _expand_instance(
	...
	var composed_transform := _compose(parent_transform, local_transform)
	...
	for primitive_index in prefab.primitives.size():
		...
		var transformed: Dictionary = _transform_primitive(resolved, composed_transform, map_origin, primitive_path, errors)
...
static func _transform_primitive(
	...
	if kind == &"terrain_stroke":
		_normalize_terrain_stroke(result, transform, origin, path, errors)
...
static func _transform_dictionary(values: Dictionary, transform: MapTransform, origin: Vector2i, translate: bool) -> void:
...
static func _transform_row_data(data: Dictionary, transform: MapTransform, origin: Vector2i) -> void:
...
static func _transform_wall_data(data: Dictionary, transform: MapTransform, origin: Vector2i) -> void:
...
static func _normalize_terrain_stroke(result: Dictionary, transform: MapTransform, origin: Vector2i, path: String, errors: Array[String]) -> void:
	var data: Dictionary = result.get("data", {})
...
static func _compose(outer: MapTransform, inner: MapTransform) -> MapTransform:
