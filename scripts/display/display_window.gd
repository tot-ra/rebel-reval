...
static func target_window_size_for_screen(
	screen_size: Vector2i, fraction: float = SCREEN_SIZE_FRACTION
) -> Vector2i:
	if screen_size.x <= 0 or screen_size.y <= 0:
		return FALLBACK_SIZE

	var maximum_size := Vector2i(
		maxi(1, int(screen_size.x * fraction)), maxi(1, int(screen_size.y * fraction))
	)
	# Preserve the design aspect ratio so viewport stretching cannot distort the UI.
	var scale := minf(
		float(maximum_size.x) / float(DESIGN_SIZE.x), float(maximum_size.y) / float(DESIGN_SIZE.y)
	)
	return Vector2i(
		maxi(1, int(round(DESIGN_SIZE.x * scale))), maxi(1, int(round(DESIGN_SIZE.y * scale)))
	)
