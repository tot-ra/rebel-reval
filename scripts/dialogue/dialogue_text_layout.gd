...
static func body_text_max_height(
	viewport_height: int, choice_count: int = 0, has_disabled_reason: bool = false
) -> int:
	...
	reserved += (
		choice_count
		* int(round(float(TextScaleScript.BASE_CHOICE_SIZE) * CHOICE_LINE_HEIGHT_FACTOR))
	)
	if has_disabled_reason:
		reserved += int(
			round(float(TextScaleScript.BASE_HINT_SIZE) * DISABLED_REASON_LINE_HEIGHT_FACTOR)
		)
...
static func multiline_text_height(font: Font, text: String, width: int, font_size: int) -> int:
	if text.is_empty():
		return 0
	return int(font.get_multiline_string_size(text, HORIZONTAL_ALIGNMENT_LEFT, width, font_size).y)
...
static func text_fits_or_needs_scroll(
	...
	var visible_height := body_text_max_height(viewport_size.y, choice_count, has_disabled_reason)
