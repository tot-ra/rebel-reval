...
| Path | Role |
|------|------|
| `kalev.tscn` | Player-view rig: `fresh_rig.gd` + `kalev_fresh/kalev_fresh/kalev_fresh.glb` + health ring |
| `kalev_variant.tres` | Stable identity `char.kalev` and shared-rig variant data |

Mesh, wardrobe `.tres` files, weapons, rebuild pipeline, and review scenes live under [`../kalev_fresh/`](../kalev_fresh/). Do not duplicate GLBs or garments here; map and save code depend on `res://assets/characters/kalev/kalev.tscn` staying thin.
