...
func set_weapon(index: int) -> void:
	...
	if weapon_index == 0:
		...
		var weapon_path := "%s%s/%s.glb" % [DIR, file, file]
		var mounted := rig.equip(&"right_hand", load(weapon_path) as PackedScene)
		assert(mounted != null)
	_update_status()
...
func _update_camera() -> void:
	var x := sin(orbit) * camera_distance
	var z := cos(orbit) * camera_distance
	var offset := Vector3(x, aim_height + 0.12, z)
...
func _process(delta: float) -> void:
	for joy: int in Input.get_connected_joypads():
		...
		var zoom := (
			Input.get_joy_axis(joy, JOY_AXIS_TRIGGER_LEFT)
			- Input.get_joy_axis(joy, JOY_AXIS_TRIGGER_RIGHT)
		)
...
func _unhandled_input(event: InputEvent) -> void:
	...
	if event is InputEventMouseButton:
		...
		if event.pressed and event.button_index in [MOUSE_BUTTON_WHEEL_UP, MOUSE_BUTTON_WHEEL_DOWN]:
			var wheel_delta := -0.2 if event.button_index == MOUSE_BUTTON_WHEEL_UP else 0.2
			camera_distance = clampf(camera_distance + wheel_delta, 0.8, 6.0)
			_update_camera()
