...
**Disposition:** source inventory and packet integrity are deterministic structural evidence, but the current-revision acceptance result remains **BLOCKED** because the packet has no stable-ID visual annotations or named canon/art sign-off. The direct Godot probe confirms that the manifest's compiled semantic fingerprint is reproducible; the raw RRMap SHA is a separate source-ledger value. Each plate now also carries sorted objective `stable_id_candidates` derived from authored footprints/landmarks intersecting its projected viewport. Those candidates make the reviewer handoff auditable, but do not count as visual observations: the current-revision acceptance result remains **BLOCKED** because the packet has no accepted stable-ID visual annotations or named canon/art sign-off. The exact Python baseline audit remains useful for checking those two revision domains are not conflated.
...
- [`docs/reports/images/lower_town_p0_101/capture_manifest.json`](images/lower_town_p0_101/capture_manifest.json), generated packet metadata, sorted objective stable-ID candidate links, and explicit `not_reviewed` observation coverage.
