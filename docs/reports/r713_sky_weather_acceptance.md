...
| `tools/run_godot_checked.sh --require-test-summary r739-save-service -- ... --filter=test_save_service` | **BLOCKED**, 0 tests, 7 errors | The same weather-script parse cascade prevents this suite from loading in the current checkout; save-service evidence is not promoted from an earlier run. |
...
| R-734 save/load persistence | `in_progress` | Save-envelope and save-service round-trip evidence passes | **BLOCKER** |
...
| Sky/cloud/weather parameters persist through save/load and map transitions | [`tests/godot/test_sky_weather_state.gd`](../../tests/godot/test_sky_weather_state.gd) covers JSON-like state round-trip between presenters; the current state and save-service suites cannot load because `SkyWeather3D` fails to parse, and R-734 remains `in_progress` | **BLOCKED runtime evidence; acceptance not complete** | R-734 owns save/load; R-735 is done, while R-714/R-738 own remaining transition/capture proof |
| Automated tests verify state continuity and no duplicate environment controllers | State tests exist; runtime tests cover shared cycle continuity; R-735 is marked done, but the current state/runtime suites are blocked before assertions by the `SkyWeather3D` parse cascade, and the save-envelope suite has a duplicate loop iterator | **BLOCKED** | R-733/R-734 must provide executable focused suites; R-735's completed owner work still requires a clean runtime verification path |
...
The contract in [`docs/SKY_WEATHER_STATE_CONTRACT.md`](../../docs/SKY_WEATHER_STATE_CONTRACT.md) correctly keeps `Environment`, `Sky`, `Camera3D`, particles, audio players, and scene nodes out of persisted state. `SkyWeather3D.snapshot_state()` / `apply_state()` and the state tests provide a useful foundation. The current Godot parse cascade prevents executable confirmation of that contract; R-732 remains `in_progress` and R-734 remains `in_progress`, so R-713 closeout is not accepted.
