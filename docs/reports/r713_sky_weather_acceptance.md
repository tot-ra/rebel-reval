...
The repository checkout was already dirty before this verification. No pre-existing changes were reverted or treated as evidence. Godot 4.7.1 is available at `/Applications/Godot.app/Contents/MacOS/Godot`, and the project import completed successfully. The focused Godot runs below are therefore reported as executed; their failures remain blockers and are not waived. Python checks were run from the current checkout on 2026-08-26.
...
| `tools/run_godot_checked.sh --require-test-summary r739-sky-weather-3d -- ... --filter=test_sky_weather_3d` | **FAIL**, 20/21 tests passed, 1 failure, 0 errors | The weather suite executes. The remaining failure is `test_sky_shader_covers_required_features`: lunar UV inset/V-flip contract. This is retained as a renderer/shader blocker, not waived. |
| `tools/run_godot_checked.sh --require-test-summary r739-sky-weather-state -- ... --filter=test_sky_weather_state` | **FAIL**, 3/4 tests passed, 1 failure, 0 errors | State validation tests execute. JSON round-trip equality fails in `test_state_round_trip_preserves_weather_continuity_inputs`; persistence continuity is not accepted. |
| `tools/run_godot_checked.sh --require-test-summary r739-save-service -- ... --filter=test_save_service` | **PASS**, 10 tests | Existing save-service round-trip and fallback behavior pass; this does not prove weather payload integration. |
| `tools/run_godot_checked.sh --require-test-summary r739-save-envelope -- ... --filter=test_save_envelope` | **BLOCKED**, script parse error | `test_save_envelope.gd` declares a duplicate `entry` loop iterator in one scope. This pre-existing test-harness defect prevents envelope verification. |
| `tools/run_godot_checked.sh --require-test-summary r739-map-view-runtime -- ... --filter=test_map_view_3d_runtime` | **BLOCKED**, 9 tests, 0 failures, 76 errors | Runtime assertions pass where reached, but repeated baseline map diagnostics include unknown destination `kuldjala_interior` and map chunk-boundary errors. The checked runner correctly rejects the suite; this is not weather acceptance evidence. |
...
|---|---|---|---|
| Physically plausible or authored sky/atmosphere synchronized with sun, ambient, fog, exposure, reflections, and day/night | [`scripts/map/view3d/sky_weather_3d.gd`](../../scripts/map/view3d/sky_weather_3d.gd) contains the sky resource, weather profiles, astronomy, and lighting modifier API; [`tests/godot/test_sky_weather_3d.gd`](../../tests/godot/test_sky_weather_3d.gd) contains source/state assertions | **PARTIAL runtime evidence**; the focused weather suite executed with 20/21 tests passing, but its shader feature contract still fails and no R-713 signed capture exists | R-736 owns synchronization; R-739 cannot accept without the shader defect being resolved and matched captures |
...
| Clear, overcast, rain, storm, and post-rain wetness with controlled transitions | [`tests/godot/test_sky_weather_3d.gd`](../../tests/godot/test_sky_weather_3d.gd) covers the five regimes, transition blending, lightning, gusts, and puddle wetness; [`docs/SKY_WEATHER_STATE_CONTRACT.md`](../../docs/SKY_WEATHER_STATE_CONTRACT.md) documents persisted fields | **PARTIAL runtime evidence**; weather regime/transition tests pass, while state JSON round-trip equality fails and save/transition integration remains open | R-733/R-734/R-736 own deterministic continuation and persistence proof |
...
| Automated tests verify state continuity and no duplicate environment controllers | State tests exist; runtime tests cover shared cycle continuity; no focused duplicate-controller test or R-735 implementation is present. The focused state suite has 3/4 tests passing; the runtime suite is rejected by 76 baseline map errors | **BLOCKED** | R-733/R-735 must provide executable focused suites and owner audit |
