...
Typed style/override keys are closed to the compiler's current semantic fields: `enabled`, `terrain`, `rect`, `wall_height`, `wall_height_scale`, `wall_color`, `roof_color`, `wall_material`, `roof_material`, `house_tier`, `vehicle_class`, `door_side`, `ridge_axis`, `primitive`, `cell`, `facing`, `style_variant`, `visual_offset_px`, `destination_scene_id`, `destination_spawn_id`, `spawn_id`, `building_id`, `spawn_offset_px`, `highlight_area`, `transition_visual`, `view_landmark_id`, `alignment`, `kind`, `direction`, `top_px`, `door_material`, `gate_variant`, `grille_variant`, `passage_axis`, `tower`, and `faction`. Each key has one parser type; unknown keys are rejected before compilation. The boolean `tower` key on a wall-kind building forces the round limestone tower dressing (drum, conical roof, arrow slits) regardless of footprint size; without it the small-footprint heuristic applies. The optional `faction` name selects readable heraldry for tower pennants, courtyard `banner` props, and merchant-cog mastheads (`danish_crown`, `livonian_order`, `hanseatic`, `harju_kings`, `black_cloaks`, `cult_metsik`, `pskov_novgorod`, `novgorod`, `pskov`, `vitalienbruder`). Prefer `novgorod` (bear) or `pskov` (lynx) when eastern trade cloth should show an animal charge; `pskov_novgorod` keeps the joint fess for undivided emissary placement. `black_cloaks` uses an invented swallow on black cloth (not the historical Brotherhood of Blackheads). `vitalienbruder` flies no flag. Confidence notes live on `FactionHeraldry`.

Monastic landmarks and primitives (`scripts/map/view3d/map_view_monastic_models.gd`) cover a pre-1343 timber house. `landmark <id> cloister_walk <rect>` builds a covered timber cloister gallery: oak posts, a stone dwarf sill, wall plate, canted braces and a lean-to shingle roof. It is a *view landmark*, never a building, so the covered walk it represents stays a walkable route. Set `passage_axis=x|z` for the run direction and `interior_side=north|south|east|west` for the range that carries the high eaves; `top_px` sets the post-plate eaves height and `wall_color` the timber tone. The house primitive `timber_oratory_1343` adds paired lancet lights, a framed west bellcote with bell, an east gable cross and its own west portal, and suppresses both the generic domestic window pass and the roof chimney. The house primitive `unheated_timber_range_1343` only suppresses the chimney, for dorters, chapter ranges and mill floors that carried no hearth. The first consumer is `content/maps/world_padise.rrmap`; see [`padise_monastery_research_p6_009.md`](reports/padise_monastery_research_p6_009.md) for the evidence bounds.

Gate landmarks support reusable generated hardware with `gate_variant=oak|ironbound|none` and `grille_variant=portcullis|none`. `door_material=wood|metal` remains a compatibility alias for `oak|ironbound`. Gate assets are always parked open or raised because `gate_arch` is a view-only landmark over a walkable passage. For the 1343 Reval circuit, prefer oak or iron-bound oak; add a single portcullis only where historical or narrative evidence exists, and do not infer later foregates, barbicans, round flanking towers, or drawbridges from the hardware choice.
...
`house_tier` is the closed R-003 ordinary Lower Town typology key on house buildings and house `style` blocks: `merchant_stone`, `merchant_timber`, or `craft_boda`. Full massing, diele/dornse depth split (~55–65% / ~35–45%), window/roof/hoist rules, rear-yard minimums, ward bias, and rejection rules live in [`docs/reports/burgher_house_typology_contract.md`](reports/burgher_house_typology_contract.md). Unknown `house_tier` values fail with the stable diagnostic `house_tier is unknown`. Reject late-Gothic tourist facade defaults (four-light stone crosses, rich blind niches), hoist hardware on `craft_boda`, and using scaled ordinary houses as exceptional landmarks. Omitting `house_tier` remains legal until district wiring assigns tiers.

`vehicle_class` is the closed R-068 merchant transport key on cart props and cart `style` blocks: `cart_2w`, `wagon_4w`, `barrow`, or `sledge`. Default urban supply vehicle is `cart_2w` (~1.3 m `wheel_rut_spacing`, `cart_path_width_min` 2.5 m on the Vanaturg throat). Full corridor allowlist, April 1343 load bands, post-23 Apr inland stall, horse-draught rule, and rejection rules (war-wagon, ox urban teams, gate toll booths, municipal dung-cart livery) live in [`docs/reports/merchant_cart_transport_contract.md`](reports/merchant_cart_transport_contract.md). Unknown `vehicle_class` values fail with the stable diagnostic `vehicle_class is unknown`. Keep `cart_toll_pfennig` null (R-069 gap - do not invent a wheel tax). Omitting `vehicle_class` remains legal until kit wiring assigns classes.
