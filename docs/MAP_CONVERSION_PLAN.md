...
| Scene | Status | Terrain palette | World bounds | Required buildings and props | Transitions and spawns | Collision and navigation | Source references | Target declarative definition |
|-------|--------|-----------------|--------------|------------------------------|------------------------|--------------------------|-------------------|-------------------------------|
| `scenes/reval_east/forge/forge.tscn` | `convert` | `forge_interior`: stone floor, packed dirt, ash, timber, iron, ember and water accents using P0-040 day and night values | 40 x 24 cells; one-screen interior envelope with camera clamp | Forge and chimney, anvil, quench bucket or trough, repair bench, ledger, bed alcove, chest, shelves, tool rack, ingots, furnace, food table, foreground wall and courtyard door | Required stable anchors: `door_courtyard`, `bed_alcove`, `anvil`; preserve legacy `main` only as a temporary cutover alias | Closed perimeter; footprint collision for furnace, bed, storage and heavy furniture; walkable work triangle; nav exclusion under foreground wall; fade volume at occluder | `scenes/reval_east/forge/forge.tscn`<br>`scenes/reval_east/forge/forge.md`<br>`scenes/reval_east/forge/Screenshot 2025-08-11 at 00.57.04.png`<br>`docs/SCENES/the-makers-mark.md`<br>`content/examples/valid/location.kalev_smithy.json` | `scripts/map/definitions/lower_town/kalev_smithy_definition.gd` |
| `scenes/reval_east/reval_east.tscn` | `convert` | `lower_town_street`: cobblestone, packed dirt, mud, drainage water, foundation stone, timber threshold, hay and sparse grass under P0-040 day and night palettes | 64 x 36 cells; bounded connected exterior replacing the legacy city-scale canvas | Smithy facade and courtyard, Foaming Mug brewery, municipal cistern or well, watch checkpoint, compact tenements and workshops, wall or Viru Gate silhouette; carts, barrels, crates, drainage, signs and evidence props | Required stable anchors: `smithy_door`, `brewery_door`, `checkpoint_west`, `checkpoint_east`, `street_start`; legacy district-edge spawn IDs remain only until atomic manifest cutover | Continuous walkable route among smithy, brewery, cistern and checkpoint; building and wall footprints; water and midden exclusion; authored checkpoint chokepoint; NPC patrol nav; Y-sort and fade for tall facades | `scenes/reval_east/reval_east.tscn`<br>`scenes/revel-map.jpg`<br>`scenes/reval_walls_towers/wall-map.png`<br>`docs/SCENES/the-makers-mark.md`<br>`docs/SCENES/a-bitter-brew.md`<br>`scenes/reval_east/foaming_mug_brewery.md`<br>`scenes/reval_east/quarters_well.md`<br>`scenes/reval_east/guards_barracks_viru.md` | `scripts/map/definitions/lower_town/lower_town_slice_definition.gd` |
...
| Scene | Status | Terrain palette | World bounds | Required buildings and props | Transitions and spawns | Collision and navigation | Source references | Target declarative definition |
|-------|--------|-----------------|--------------|------------------------------|------------------------|--------------------------|-------------------|-------------------------------|
| `scenes/reval_center/reval_center.tscn` | `convert` | `market_civic_prototype`: worn cobble, dressed stone, packed dirt, timber threshold and drainage water | 64 x 36 cells | Town Hall mass, square edge, Holy Spirit church or hospital silhouette, apothecary, guild frontage, stalls, carts, well and civic notice props | Prototype inspection spawn only; no doors to active slice and no manifest entry | Closed prototype perimeter, landmark footprints, square circulation nav and Y-sort; no quest patrol graph | `scenes/reval_center/reval_center.tscn`<br>`scenes/revel-map.jpg`<br>`scenes/reval_walls_towers/wall-map.png`<br>`scenes/reval_center/market_civic_quarter/town_hall_square.md`<br>`scenes/reval_center/apothecary.md` | `scripts/map/definitions/prototypes/market_civic_quarter_definition.gd` |
| `scenes/reval_center/market_civic_quarter/market.tscn` | `convert` | `market_square_prototype`: worn cobble, packed dirt, mud, timber and canvas accents | 50 x 28 cells | Market lanes, covered stalls, weighing table, well, carts, baskets, barrels, awnings and Town Hall edge | Prototype inspection spawn only; no active destination | Stall footprints, clear crossing routes, crowd nav lanes, no trapping pockets, shared Y-sort | `scenes/reval_center/market_civic_quarter/market.tscn`<br>`scenes/reval_center/market_civic_quarter/market.md`<br>`scenes/reval_center/market_civic_quarter/town_hall_square.md` | `scripts/map/definitions/prototypes/market_square_definition.gd` |
...
| `scenes/reval_north/reval_north.tscn` | `convert` | `north_quarter_prototype`: cobble, packed dirt, stone, timber, dock mud and sparse grass | 64 x 36 cells | Pikk Street spine, St. Olaf's silhouette, guild facades, workshops, shipwright or rope props, carts and barrels | Prototype inspection spawn only; all five legacy district-edge spawn IDs retire rather than connect | Closed prototype perimeter, landmark footprints, street nav spine, workshop exclusions and Y-sort | `scenes/reval_north/reval_north.tscn`<br>`scenes/revel-map.jpg`<br>`scenes/reval_north/pikk_street.md`<br>`scenes/reval_north/st_olafs_church.md`<br>`scenes/reval_north/great_guild_hall.md` | `scripts/map/definitions/prototypes/north_quarter_definition.gd` |
...
4. **Archive empty and visual shells.** P0-045 and P0-046 preserve markdown and image sources while removing obsolete `.tscn` shells from runtime paths. This step does not wait for production art.
