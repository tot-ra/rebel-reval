...
| `scenes/reval_north/oleviste_church/oleviste_church.tscn` | level | `convert` | `loc.lower_town.st_olafs_church` | `active=false` prototype only until separate approval artifact | `scripts/map/definitions/prototypes/oleviste_church_definition.gd` |
...
| `scenes/reval_north/oleviste_church/oleviste_church.tscn` | `convert` | `church_interior_prototype`: limestone nave, altar apron, candle and bench dressing | 36 x 24 cells | St. Olaf nave, altar block, baptismal font, lectern stand, benches and south entry | Reciprocal developer transition from Monastery District `st_olaf_silhouette`; `active=false` until separate approval artifact | Closed interior shell, nave circulation nav and foreground fade | `content/maps/oleviste_church.rrmap`<br>`scenes/reval_north/oleviste_church/oleviste_church.tscn`<br>`scenes/reval_north/st_olafs_church.md` | `scripts/map/definitions/prototypes/oleviste_church_definition.gd` |
