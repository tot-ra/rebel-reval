...
- `WorldMapOverlay` renders local-map and district-graph state and emits `travel_requested`; `WorldMapController` executes travel through `DoorNavigator`.
...
| `scripts/ui/world_map_overlay.gd` (601) | Full-screen local map, district graph, tabs, graph layout/drawing, focus navigation, and travel intent | Two independently changing presentation modes in one overlay. This is the first UI extraction candidate: move local-map and fast-travel rendering into focused child views while preserving `WorldMapOverlay` and controller-owned travel. Protect with all `test_world_map_overlay` cases and `test_quick_access_menu`. |
...
P1-034 in `TODO.md` owns the first extraction identified by this audit: split the local-map and fast-travel presentation internals of `WorldMapOverlay` while preserving its public facade and controller-owned side effects. Other candidates remain thresholds, not scheduled rewrites. They require a new or updated TODO contract before implementation.
