...
| `scripts/map/view3d/map_view_runtime.gd` | 714 | **Partial extract closed (P0-185).** Ambient installers (birds/fauna/insects/crowd/music) live in `map_view_runtime_ambient.gd`; actors and camera remain in sibling helpers. Next peel: time-flow controls if they change independently. Gate: `test_map_view_3d_runtime`, runtime-camera, click, crowd, fauna, and session-state replacement filters. |
