...
| --- | ---: | --- |
| `scripts/map/view3d/map_view_mesh_builder_prop_models.gd` | 503 | **Partial extract closed (P0-185).** Kalev smithy workstation builders live in `map_view_smithy_prop_builder.gd`; this file keeps the `build_prop` facade and outdoor/boat/fauna branches. Gate: `test_map_view_3d_mesh`, `test_environment_kit_catalog`, `test_street_and_masonry_realism`. |
...
| `scripts/map/view3d/map_view_runtime.gd` | 439 | **Partial extract closed (P0-185).** District install wiring and player readability fill light live in `map_view_runtime_bootstrap.gd`; ambient installers live in `map_view_runtime_ambient.gd`; time-flow controls in `map_view_runtime_time_flow.gd`; sky clock, calendar, MusicDirector sync, and SessionState weather binding in `map_view_runtime_environment.gd`; SessionState equipment binding in `map_view_runtime_session.gd`; flat 2D art hiding in `map_view_runtime_flat_map.gd`; click routing and camera zoom gestures in `map_view_runtime_input.gd`. Actors and camera remain in sibling helpers. Gate: `test_map_view_3d_runtime`, `test_debug_overlay`, `test_session_state_replacement`, `test_r715_water_map_handoff`, `test_map_click_input_controller`, `test_map_view_runtime_camera`, crowd, fauna filters. |
...
| File (lines) | Decision and regression gate |
| --- | --- |
| `scripts/map/view3d/map_view_materials.gd` (358) | **Partial extract closed (P0-185).** Wind-driven materials live in `map_view_wind_materials.gd`; dry terrain, blended-ground splats, and mud wetness live in `map_view_terrain_materials.gd`; the closed `WATER_WAVE_BASE` catalog and `WATER_TERRAINS` alias live in `map_view_water_materials.gd`; building and prop caches remain in sibling modules (`role_for_size` sizing lives in `map_view_prop_materials.gd`). This file keeps public facades and weather-presentation wiring. Gate: `test_map_view_material_resolution`, `test_map_terrain_chunks`, `test_street_and_masonry_realism`, `test_grass_interaction`, `test_vegetation_realism`, `test_fishing_nets`, `test_faction_heraldry`, `test_boat_float_3d`, `test_r715_water_material_contract`. |
...
| `scripts/map/view3d/map_view_runtime_camera.gd` (211) | **Keep** camera modes beside the runtime facade; follow in `map_view_runtime_camera_follow.gd`, orbit in `map_view_runtime_camera_orbit.gd`, modes in `map_view_runtime_camera_modes.gd`, safety in `map_view_runtime_camera_safety.gd`, third-person target resolution in `map_view_runtime_camera_target.gd`, zoom continuum in `map_view_runtime_camera_zoom.gd`, screen shake in `map_view_runtime_camera_shake.gd`, and perspective exposure/DOF in `map_view_runtime_camera_perspective.gd`. Gate: `test_map_view_3d_runtime`, `test_map_view_runtime_camera`, `test_map_camera_modes`, and camera/click filters. |
...
- P0-185 (partial): `map_view_terrain_materials.gd` now owns dry-terrain caches, blended-ground splats, and mud-wetness updates; `map_view_materials.gd` dropped from 653 to 453 lines and remains the public facade. Gate: `test_map_view_material_resolution`, `test_map_terrain_chunks`, `test_street_and_masonry_realism`, `test_natural_ground_variation`.
...
- P0-185 (partial): `map_view_water_materials.gd` now owns the closed `WATER_WAVE_BASE` catalog; `map_view_materials.gd` dropped from 468 to 410 lines and re-exports the catalog for contract tests. Gate: `test_r715_water_material_contract`, `test_r715_water_weather_sync`, `test_coastal_sea_3d`, `test_map_view_material_resolution`.
- P0-185 (partial): `map_view_runtime_camera_follow.gd` now owns snap/lerp follow and mode-specific boom targets; `map_view_runtime_camera.gd` dropped from 237 to 211 lines and remains the public mode/orbit/zoom facade. Gate: `test_map_view_runtime_camera`, `test_map_camera_modes`, `test_map_view_3d_runtime`.
- P0-185 (partial): `map_view_runtime_camera_modes.gd` now owns per-mode presentation and cycling; `map_view_runtime_camera.gd` dropped to 237 lines before the follow peel. Gate: `test_map_view_runtime_camera`, `test_map_camera_modes`, `test_map_view_3d_runtime`.
- P0-185 (partial): `map_view_runtime_camera_orbit.gd` now owns keyboard/mouse yaw and pitch orbit; `map_view_runtime_camera.gd` dropped from 330 to 291 lines before later peels. Gate: `test_map_view_runtime_camera`, `test_map_camera_modes`, `test_map_view_3d_runtime`.
- P0-185 (partial): `map_view_runtime_camera_zoom.gd` now owns scroll/pinch/pan zoom and the third-person / first-person / top-down distance continuum; `map_view_runtime_camera.gd` dropped from 374 to 330 lines before later peels. Gate: `test_map_view_runtime_camera`, `test_map_camera_modes`.
- P0-185 (partial): `map_view_runtime_camera_shake.gd` now owns trauma decay, accessibility-gated shake requests, and follow-position offset; `map_view_runtime_camera.gd` dropped from 407 to 374 lines before the zoom peel. Gate: `test_map_view_runtime_camera`, `test_map_camera_modes`.
- P0-185 (partial): `map_view_runtime_camera_perspective.gd` now owns practical-camera exposure and depth-of-field for perspective modes; `map_view_runtime_camera.gd` dropped from 454 to 405 lines and remains the public mode/follow owner. Gate: `test_map_view_runtime_camera`, `test_map_camera_modes`.
- P0-185 (partial): `map_view_runtime_camera_target.gd` now owns third-person follow-target resolution and interior floor-edge clamping; `map_view_runtime_camera.gd` dropped from 508 to 454 lines. Gate: `test_map_view_runtime_camera`, `test_map_camera_modes`.
- P0-185 (partial): `map_view_runtime_camera_safety.gd` now owns post-follow ground clamp, building pull-out, visibility recovery, and occlusion ghost; `map_view_runtime_camera.gd` dropped from 678 to 508 lines and remains the public mode/follow owner. Gate: `test_map_view_3d_runtime`, `test_map_view_runtime_camera`, `test_map_camera_modes`, `test_map_click_input_controller`.
- P0-185 (partial): `map_view_runtime_bootstrap.gd` now owns `MapViewRuntime.install()` wiring and the player readability fill light; `map_view_runtime.gd` dropped from 524 to 439 lines and remains the public facade. Gate: same runtime filters as the input peel.
- P0-185 (partial): `map_view_runtime_input.gd` now owns MapClickInput install and unhandled camera/time input; `map_view_runtime.gd` dropped from 580 to 524 lines and remains the public facade. Gate: `test_map_view_3d_runtime`, `test_debug_overlay`, `test_session_state_replacement`, `test_r715_water_map_handoff`, `test_map_click_input_controller`, `test_map_view_runtime_camera`.
- P0-185 (partial): `map_view_prop_materials.gd` now owns size-aware `role_for_size` UV scaling beside `role()`; `map_view_materials.gd` dropped from 380 to 370 lines and remains the public facade. Gate: `test_map_view_material_resolution`, `test_street_and_masonry_realism`, door/landmark mesh filters that call `MapViewMaterials.role_for_size`.
- P0-185 (partial): `map_view_runtime_session.gd` and `map_view_runtime_flat_map.gd` now own SessionState equipment binding and flat 2D art hiding; `map_view_runtime.gd` dropped from 656 to 580 lines and remains the public facade. Gate: `test_map_view_3d_runtime`, `test_debug_overlay`, `test_session_state_replacement`, `test_r715_water_map_handoff`.

### Scheduled follow-up

**P0-185** performs justified extractions only for the remaining extraction targets in the 800+ table (`map_view_runtime`, and any independently justified concern in `map_view_materials`/`game_state` after review). `map_view_mesh_builder_prop_models` smithy-kit builders are closed in `map_view_smithy_prop_builder.gd`. `map_view_tree_meshes` profile tables are closed in `map_view_tree_mesh_profiles.gd`; `sky_weather_3d` and `map_view_3d` remain cohesive integration owners; the bird, mammal, and material-shader catalog shards are closed. Do not open broad LOC-driven rewrites of the 400-799 bands. Documentation/agent readability slim-downs for `docs/MAP_AUTHORING.md`, `docs/ROADMAP.md`, and offline `tools/` generators are tracked beside storage work in [`docs/STORAGE_SIZE_BACKLOG.md`](./STORAGE_SIZE_BACKLOG.md) and the 2026-08-13 readability report; they are not runtime architecture extractions.
