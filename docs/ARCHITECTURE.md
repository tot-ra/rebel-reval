...
| File (lines) | Decision and regression gate |
| --- | --- |
| `scripts/map/view3d/map_view_materials.gd` (410) | **Partial extract closed (P0-185).** Wind-driven materials live in `map_view_wind_materials.gd`; dry terrain, blended-ground splats, and mud wetness live in `map_view_terrain_materials.gd`; the closed `WATER_WAVE_BASE` catalog lives in `map_view_water_materials.gd`; building and prop caches remain in sibling modules. This file keeps public facades and weather-presentation wiring. Gate: `test_map_view_material_resolution`, `test_map_terrain_chunks`, `test_street_and_masonry_realism`, `test_grass_interaction`, `test_vegetation_realism`, `test_fishing_nets`, `test_faction_heraldry`, `test_boat_float_3d`, `test_r715_water_material_contract`. |
