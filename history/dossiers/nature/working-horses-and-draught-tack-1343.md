...
- No reviewed source supplies a 1343 Reval horse skeleton with body measurements, sex, coat, or work specialization [1][2][3]. This is an evidence gap; assigning exact values here would be `invented`, not research.
...
- Hoof treatment can read as plain and work-worn. A 1200–1400 English horseshoe is a chronological comparandum in the existing craft evidence, not proof of a Livonian form [5]. This is `plausible composite`; exact Reval shoe dimensions are an evidence gap and any fixed local measurement would be `invented`.
- Use neutral coat slots and restrained markings only. Coat frequency, mane length, leg white, and sex-linked colour differences are an evidence gap for Reval 1343 [1][2]; a fixed distribution would be `invented`.
...
- The visible tack set should therefore be minimal: bridle or bit when steering is needed, a plain leather collar or breast support, traces, shafts, and simple fastening points. Exact collar, hames, pad, and strap geometry in Reval 1343 is an evidence gap; the sparse set is `plausible composite` from the cart brief and northern-European comparanda [4][7].
- Decorative brass harness, modern synthetic webbing, Western saddle styling, and mounted-warhorse armour are explicit exclusions. No source reviewed here supports them for a working cart horse in Spring 1343 [3][4]. This is `invented` as a production exclusion, not a historical claim.
...
- **Smithy pairing:** A plain shoe can be a repair prop or forge commission; do not turn the comparative English shoe into a local archaeological claim [5].
...
- [`../crafts/blacksmith-materials-and-techniques.md`](../crafts/blacksmith-materials-and-techniques.md) - horseshoe evidence and forge/repair pairing.
