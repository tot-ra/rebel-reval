...
**R-499 recheck (2026-08-13, 13:03 UTC):** separate direct `curl -L` checks reproduced the same zero-folio boundary. The official AIS unit URL returned HTTP 200 `text/html` catalogue content containing `TLA.230.1.Aa2` and anonymous `Gallery image 0`-`25` markers, but no folio identifiers, original wording, or usable transcription. The DGS permalink returned HTTP 200 `text/html` only after six redirects, ending at the VAU login URL and exposing the protected TIFF target `tla0230_001_0000aa2_00001_x.tif`. Both responses were verified as HTML rather than manuscript image data. No authenticated session or archive-supplied scan/transcription was available, so zero Denkelbuch folios were inspected. This remains an access blocker, not a carter-owner hit or manuscript no-hit; `named_carter_owner` remains `null`, and the authorized-access handoff remains **R-462** with access-enablement details in **R-502**.
