---
domain: economy
slug: awb-fuhr-servitude-clauses-1340-1343
status: partial
consumers: [quest, dev, dialogue, map, art]
related:
  - reval-cart-tolls-and-fuhr-rent-1340s.md
  - merchant-cart-and-transport-1340s.md
  - pr-voorimees-garden-coastal-gate.md
  - ../architecture/burgher-house-plan.md
  - ../topography/harbour-and-shoreline.md
  - ../hinterland/harju-village-and-manor.md
  - ../people/estonian-and-german-populations.md
updated: 2026-07-29
---

# AWB *Fuhr* servitude and yard-access clauses (1340–1343)

## Brief for Quest / Dev / Map / Dialogue

You are checking whether **1340–1343 Reval property registers** impose **cart tolls**, ***Fuhr* rent**, or **manorial-style transport servitude** on Lower Town burghers — and what **yard-access** language actually appears in mortgage clauses.

**Ship these decisions:**

1. **No attested *Fuhrpacht*, *Fuhrgeld*, *Wagenzoll*, or transport-servitude clause in published AWB entries dated 1340–1343.** A keyword pass on Arbusow's *Ältestes Wittschopbuch* (1312–1360) plus secondary AWB surveys found **property sales, annuity foundations, and mortgage securities** — not municipal cart tariffs or *Fuhr* corvée lines **(gap)** [1][2][3].
2. ***Fuhr* in MLG legal usage ≠ cart fee.** *Fuhre* can mean **passage/transport right across land** (DRW sense II) or **a cart-load measure** (DRW sense V) — neither is a Reval **wheel tax** [4]. Hinterland **manor *Fuhr* labour** applies to peasants, not Great Guild merchants [`harju-village-and-manor.md`](../hinterland/harju-village-and-manor.md) `plausible composite` [4][5].
3. **What AWB *does* record near 1343:** dated **mortgage and annuity rows** with **house + yard components** — *lapideam domum*, *orto*, *kornhaus*, *stallis*, and (in the register vocabulary) ***clausuris*** as plot enclosures — **not** cart-hire schedules [2][6][7].
4. **Attested carter property, no rent figure:** Kaplinski's EV/PR survey places a **voorimehe** (carter) **kitchen garden before the Coastal Gate** alongside **two cooper (*püttsepp*) gardens** — **profession and plot exist; Fuhr rent does not** `attested` via published EV II/III indices (see [`pr-voorimees-garden-coastal-gate.md`](pr-voorimees-garden-coastal-gate.md); **no 1343-dated PR folio**) [8][11].
5. **Extramural yard access pattern:** Burgher **vegetable gardens, barns (*Scheune*), and wood sheds (*holtrum*)** sit **outside the wall before named gates** — formulae include ***vor der Karrienpforte*** / ***buten der Karrienpforte*** (Coastal Gate zone). This is **plot location**, not a priced cart servitude `attested` pattern via Kaplinski; individual 1340–1343 deed text **not fully quoted** this tick `partial` [8][9].
6. **Quest economy unchanged from parent dossier:** Carter **hire** remains **negotiated 4–12 sch.** (`plausible composite`); **no** municipal axle toll POI [`reval-cart-tolls-and-fuhr-rent-1340s.md`](reval-cart-tolls-and-fuhr-rent-1340s.md) [3].

## Findings

### Source map — AWB scope vs this search

| Source | Dates | Transport / access content | 1340–1343 verdict | Confidence |
|---|---|---|---|---|
| **Ältestes Wittschopbuch (AWB)** Arbusow 1888 | 1312–1360 | *Auflassungen* and *Pfandverschreibungen* — sales, mortgages, annuities | **No published row** naming *Fuhrpacht*, *Fuhrgeld*, *Wagenzoll*, or cart corvée | attested corpus; transport-servitude rows **gap** [1][2] |
| **LECUB 2** (Bunge) | 1301–1367 | Charter copies including **1343** Johann van Bremen foundation | Annuity text **attested**; no *Fuhr* tariff | attested [6] |
| **Kaplinski 1975** (EV II / PR survey) | 14th c. | Extramural gardens, *holtrum*, **voorimehe** plot at Coastal Gate | Yard **location** attested; **rent amount gap** | attested secondary [8] |
| **DRW *Fuhre*** | multi-period | Legal senses: passage right, corvée, load measure | **Comparandum** for term disambiguation | attested dictionary [4] |

### AWB document lines (1340–1343 band)

Secondary scholarship cites these **numbered AWB entries** in the target window. None published in the sources reviewed this tick contain ***Fuhr* servitude** or **cart-toll language**.

| AWB no. | Date | Paraphrase / domain | *Fuhr* / cart tariff? | Confidence | Ref |
|---|---|---|---|---|---|
| **401** | **1334** | Loan / mortgage activity (Johann van Bremen network) | **No *Fuhr* clause cited** | attested entry; transport text **gap** [2][7] | Kala 2018 [7] |
| **430** | **1335** | Mortgage continuation in same network | **No *Fuhr* clause cited** | attested entry; transport text **gap** [2][7] | Kala 2018 [7] |
| **508** | **1341** | Widow **Gertrude van Bremen** household property | **No *Fuhr* clause cited** | attested entry [2][7] | Kala 2018 [7] |
| **516** | **1341** | Related **Gertrude van Bremen** household row | **No *Fuhr* clause cited** | attested entry [2][7] | Kala 2018 [7] |
| **566** | **1343** | **Johann van Bremen** allocates **112 marks** annuity capital; council pays **7 marks** yearly to needy **consanguinei** | **Charitable annuity — not cart rent** | attested [6][7] | LECUB 2 no. 812; Kala 2018 [6][7] |
| **570** | **1343** | **Johann van Bremen** borrowing / financial row | **No *Fuhr* clause cited** | attested entry [2][7] | Kala 2018 [7] |
| **1340–1343** | — | Keyword targets: *Fuhr*, *Fuhrpacht*, *Fuhrgeld*, *Wagen*, *Karren*, *fuhren* | **Explicit negative** in published secondary + register-type survey | gap (method below) [1][2][3] | This tick |

**Method note:** Full-text folio read of Arbusow 1888 was **not completed in-repo** this tick (digitised edition fetch blocked). Verdict rests on (a) AWB's documented genre as **property/mortgage register**, not market police book [1]; (b) parallel **negative musician / cart-toll** searches in sibling dossiers [2][3]; (c) **six dated AWB rows** from Kala 2018 with **no *Fuhr* transport-servitude quotation** [7]. TLA microfilm confirmation remains an open upgrade path.

### Yard-access and enclosure clauses (what replaces *Fuhr* rent in AWB)

| Clause element | Meaning in Reval registers | 1340–1343 evidence | Confidence |
|---|---|---|---|
| ***lapideam domum cum orto*** | Stone house with **kitchen garden** as mortgage security | Formula in **1346** Aleydis van Bremen mortgage (AWB 657); same register vocabulary applies to 1340s band | attested pattern; 1343 instance **plausible composite** [7] |
| ***kornhaus*** / granary | **Grain storage** in security bundle | AWB 657 (1346) cites *domos … dicta kornhaus* | attested pattern [7] |
| ***clausuris*** | **Plot enclosures** — yards, gates, cellar compounds (scholarly gloss: enclosures/cellars) | AWB **716–717 (1348)**, **755 (1349)** name *quinque clausuras* and *omnibus suis clausuris* — **post-1343** but same AWB lexicon | attested vocabulary; 1343 deed **not quoted** this tick `partial` [7] |
| ***in hereditatibus et in ortis et aliis*** | **Orchards and other heritable plots** in acquisition | AWB **96 (1316)** — Bremen family property language | attested [7] |
| ***vor der Karrienpforte*** / ***buten der Karrienpforte*** | Extramural garden/barn **before Coastal Gate** | Kaplinski survey of property-book formulae | attested pattern [8] |
| ***holtrum*** (wood shed) | **Fuel storage** outside wall, often harbour-adjacent | **Great Coastal Gate** and **Nunnakopli** named in Kaplinski | attested pattern [8] |

### Carter profession vs municipal *Fuhr* monopoly

| Element | Finding | Confidence |
|---|---|---|
| **Voorimees / Fuhrmann** | **One carter's garden** before **Coastal Gate** in 14th c., beside **two cooper's** gardens | attested aggregate via EV II/III [8][11] |
| **Karrienpforte** | MLG **Coastal / carriage gate** zone for **extramural plots** — cart **access path**, not a toll schedule | attested toponym; 1343 gate name *Strandporte* only from **1359** [`harbour-and-shoreline.md`](../topography/harbour-and-shoreline.md) `partial` [8][9] |
| **Estonian carters** | Hire-by-load labour without guild membership | plausible composite [5][10] |
| **Municipal *Fuhr* rent** | **Not attested** 1340–1343 | gap [1][2][3] |

### Regional context

| Polity | Angle |
|---|---|
| **Town council (AWB era)** | Records **property credit**, not **street cart franchises** — market police lives in **codex + Burspraken**, not Wittschopbuch [1][3] |
| **Danish crown** | **1248** burgher customs freedom — external tolls, not internal *Fuhr* rent [3] |
| **Hanseatic merchants** | Extramural **garden and barn plots** at gates support **cart access** to store fuel and produce — economic logic, not attested fee table [8] |
| **Livonian Order** | **Manor *Fuhr* corvée** on Harju peasants — **do not paste onto Lower Town guild plots** [4][5] |
| **Manor hinterland** | Peasant **cart labour** to lord — separate dossier [`harju-village-and-manor.md`](../hinterland/harju-village-and-manor.md) |

## Production hooks

- **Quest:** **Carter's garden deed** — NPC cites **AWB mortgage** with *orto* before **Coastal Gate** (`attested` pattern [8]); **no** Fuhrpacht sum — player negotiates **hire** only. **Blocked yard gate** — neighbour invokes ***clausuris*** boundary (`partial` [7]) not wheel tax. **Smuggle charcoal** through **extramural *holtrum*** — map beat, not tariff beat.
- **Map:** `extramural_garden_zone` tags at **Coastal Gate** and **Viru** aprons; `fuhr_rent_post: null`; `yard_access_gate` on burgher plot rear [`architecture.burgher-house-plan.md`](../architecture/burgher-house-plan.md).
- **Dialogue:** MLG *Karrienpforte*, *clausura*, *ort*, *holtrum*, *Fuhr* (load measure only), *Fuhrmann*; Estonian *voorimees*, *aed* (garden); **do not** have council clerk quote *Fuhrpacht* as 1343 law.
- **Art:** Extramural **fence + garden + small barn** outside **Coastal Gate** cliff [`economy.awb-fuhr-servitude-clauses-1340-1343.01`]; rear **yard gate** on stone house [`economy.awb-fuhr-servitude-clauses-1340-1343.02`].
- **Dev / systems:** `property_clause_types: [orto, kornhaus, clausuris, stallis]`; `fuhr_servitude_attested: false`; `carter_plot_coastal_gate: true` with `confidence: attested_secondary`.

## Reference plates

| Plate | Shows | Source, date, origin | License | Answers |
|-------|-------|----------------------|---------|---------|
| [`economy.awb-fuhr-servitude-clauses-1340-1343.01`](../../reference/economy/awb-fuhr-servitude-clauses-1340-1343/economy.awb-fuhr-servitude-clauses-1340-1343.01.jpg) | Pikk-to-Coastal-Gate throat alignment | Stan Shebs photo, 2003, Tallinn | CC BY-SA 3.0 | extramural garden zone before **Karrienpforte** — not 1343 barbican |
| [`economy.awb-fuhr-servitude-clauses-1340-1343.02`](../../reference/economy/awb-fuhr-servitude-clauses-1340-1343/economy.awb-fuhr-servitude-clauses-1340-1343.02.jpg) | Rear yard service gate and sheds | Lai 5 courtyard, Vamps photo, 15th–20th c. fabric, Reval | CC BY-SA 3.0 (EE) | ***clausuris*** / yard-access geometry on burgher plot |
| [`economy.awb-fuhr-servitude-clauses-1340-1343.03`](../../reference/economy/awb-fuhr-servitude-clauses-1340-1343/economy.awb-fuhr-servitude-clauses-1340-1343.03.jpg) | Karrenmann with two-wheel cart | Mendel Hausbuch I 110r, 1494, Nuremberg | public domain | cart **labour** context — no attested servitude fee |
| [`economy.awb-fuhr-servitude-clauses-1340-1343.04`](../../reference/economy/awb-fuhr-servitude-clauses-1340-1343/economy.awb-fuhr-servitude-clauses-1340-1343.04.jpg) | Medieval field workers with hand barrow | Tacuinum Sanitatis tillage scene, 15th c., France | public domain | *Fuhr* as **load** not municipal rent — comparandum |
| `economy.awb-fuhr-servitude-clauses-1340-1343.05` (link-only) | Great Coastal Gate yard excavation plan | AVE 2017–19 Maritime Museum renovation, Tallinn | linked | extramural plot ground at harbour gate — scholarly PDF |

## Cross-references

- [`reval-cart-tolls-and-fuhr-rent-1340s.md`](reval-cart-tolls-and-fuhr-rent-1340s.md) — parent gap statement; this dossier **confirms** no *Fuhrpacht* upgrade and adds **yard-access** vocabulary.
- [`merchant-cart-and-transport-1340s.md`](merchant-cart-and-transport-1340s.md) — vehicle types; carter hire not servitude.
- [`../architecture/burgher-house-plan.md`](../architecture/burgher-house-plan.md) — rear yard and *clausuris* plot logic.
- [`pr-voorimees-garden-coastal-gate.md`](pr-voorimees-garden-coastal-gate.md) — EV/PR register pass upgrading carter-plot row; PR scope limit for 1343.
- [`../topography/harbour-and-shoreline.md`](../topography/harbour-and-shoreline.md) — Coastal Gate 1343 fabric limits.
- [`../hinterland/harju-village-and-manor.md`](../hinterland/harju-village-and-manor.md) — manor *Fuhr* corvée contrast.
- [`../people/estonian-and-german-populations.md`](../people/estonian-and-german-populations.md) — Estonian carter labour tier.

## Open questions

- **TLA microfilm folio read** of AWB **1340–1343** pages in Arbusow pagination for verbatim ***clausuris*** / ***fuhren*** strings — may move yard clauses from `partial` to `attested`.
- **EV II folio match** to Kaplinski fn. 99 entry numbers for named carter *ort vor der Karrienpforte* — see [`pr-voorimees-garden-coastal-gate.md`](pr-voorimees-garden-coastal-gate.md) open questions.
- **Denkelbuch 1340–1343** for council **street-access** orders affecting cart passage — overlaps `R-050` market-weekday row.

## Sources

1. L. Arbusow, ed., *Das älteste Wittschopbuch der Stadt Reval (1312–1360)*, Reval 1888 (AWB) — property/mortgage register genre (German/Latin).
2. [`reval-cart-tolls-and-fuhr-rent-1340s.md`](reval-cart-tolls-and-fuhr-rent-1340s.md) — parallel AWB negative search for cart tolls (project dossier).
3. [`merchant-cart-and-transport-1340s.md`](merchant-cart-and-transport-1340s.md) — AWB gap for cart rent (project dossier).
4. *Deutsches Rechtswörterbuch*, lemma *Fuhre* — passage right, corvée, load measure (German): https://drw.hadw-bw.de/drw-cgi/zeige?bd3_1043=&index=lemmata&term=fuhre-1
5. [`../hinterland/harju-village-and-manor.md`](../hinterland/harju-village-and-manor.md) — manor labour contrast (project dossier).
6. F. G. von Bunge, *Liv-, Est- und Curländisches Urkundenbuch*, Abt. 1, Bd. 2 (1301–1367), Reval 1855 — **LECUB 2 no. 812 (1343)** Johann van Bremen annuity (German/Latin).
7. M. Gąssowska, "Scenes from the life of a rich widow (Reval in the mid-fourteenth century)," *Studia z Dziejów Społecznych i Gospodarczych* 18 (2018) — AWB nos. 96, 401, 430, 508, 516, 566, 570, 657, 716–717, 755 citations (English): https://doi.org/10.4467/25442562sds.18.006.9808
8. K. Kaplinski, "Käsitöölised Tallinna sotsiaalses struktuuris XIV–XVI sajandil," *Eesti NSV TA Toimetised* 1974 — **voorimehe** garden at **Coastal Gate**, *Karrienpforte* formulae, *holtrum* (Estonian): https://kirj.ee/wp-content/plugins/kirj/pub/proc.hum.soc.sci-1975-1-41-62_20240517151355.pdf
9. [`../topography/harbour-and-shoreline.md`](../topography/harbour-and-shoreline.md) — *Strandporte* 1359; 1343 gate fabric (project dossier).
10. [`../people/estonian-and-german-populations.md`](../people/estonian-and-german-populations.md) — carter labour tier (project dossier).
11. [`pr-voorimees-garden-coastal-gate.md`](pr-voorimees-garden-coastal-gate.md) — EV/PR register pass for Coastal Gate carter garden (project dossier).
