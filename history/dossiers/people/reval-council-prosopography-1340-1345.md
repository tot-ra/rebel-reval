---
domain: people
slug: reval-council-prosopography-1340-1345
status: partial
consumers: [character, narrative, quest, dialogue]
related:
  - town-council-and-officers.md
  - ../language/names-address-and-oaths.md
  - ../power/jurisdictions-of-reval.md
  - ../economy/awb-clausuris-mortgage-text-1340-1343.md
updated: 2026-08-12
---

# Reval council prosopography, 1340-1345

## Brief for Character / Narrative / Quest / Dialogue

This dossier separates dated council evidence from annual council lists. The public-domain AWB edition identifies
several men as `consulatus socius` or `consul` in entries dated 1341, 1343, 1344, and 1345. A traceable
secondary transcription by F. G. von Bunge also reproduces the annual `sitzender Rat` lists for 1340-1345
and discusses their rotation. The Bunge lists materially narrow the 1343-1345 bench, but they remain a
nineteenth-century secondary witness, not a folio-level read of TLA.230.1.Aa2. The dossier still does **not**
establish a securely named burgomaster pair from the Denkelbuch itself.

**Ship these decisions:**

1. Use **Johannes de Breme**, **Hermannus More**, **Wynandus Longus**, **Bertoldus Bickinchusen**,
   **Reynekinus Crowel**, **Detmarus de Unna**, **Hunoldus de Ostinchusen**, **Remboldus de Lenepe**,
   and **Arnoldus** as dated council-member candidates only when the cited entry is named [1].
2. For a 1343 council scene, the Bunge list supports the sitting bench **Hermannus Moren, Wynandus
   Longus, Hermannus Stympel, Theodericus de Unna, Johannes Cracht, Johannes de Bremen, Johannes Albus,
   Everhardus de Unna, Hinricus Crowel, Thidemannus Cosfelt, Arnoldus Colner, Hermannus Weldege, Johannes
   Hamer, Johannes Dunevar, and Johannes de Lippia**. Label this a secondary `sitzender Rat` list, not a
   complete Denkelbuch election transcription [9].
3. The first two positions in Bunge's annual lists are a strong secondary indicator of the two leading
   `proconsules`: **Hermannus Moren / Wynandus Longus** in 1343, **Reynekinus Crowel / Wennemarus Holloger**
   in 1344, and **Wynandus Longus / Hermannus Moren** in 1345. Do not present these as a securely collated
   1343-1345 burgomaster pair: Bunge also says that the wider old and new councils could contain up to five
   proconsules in 1342-1343 [9].
4. Keep **Dietrich (Didericus) Zierenberg** as the one near-1343 burgomaster identified by the
   existing council dossier, with death on 6 December 1345; this file does not independently upgrade
   the burgomaster claim [4][5].
5. Do not present an AWB `consulatus socius` as a burgomaster. The phrase proves council membership or
   council association in that entry, not which chair the man held [1].
6. The 1333 cohort remains a dated name stock, not proof that all fourteen councillors still sat in
   1343. Mark continued membership as `plausible composite` unless a dated council-book entry is supplied [3][6].
7. The exact December 1343 election outcome and a manuscript-confirmed burgomaster pair remain an
   archival gap pending a folio-level read of Denkelbuch TLA.230.1.Aa2 [2].

## Findings

### Source and method boundary

The accessible AWB edition is Leonid Arbusow's *Das älteste Wittschopbuch der Stadt Reval
(1312-1360)* (Reval, 1888). It is a property and mortgage register, not a complete council
membership list. The entries below are transcribed from the public-domain MDZ OCR and checked against
the corresponding page OCR; the printed page and AWB number are retained so a later manuscript
collation can correct OCR readings [1].

The ILC catalogue identifies the first Denkelbuch volume as **TLA.230.1.Aa2**, 48 pages,
1333-1374, with notes on council elections, punishments, and judgments. The catalogue supplies the
archive reference and period, but its public page does not expose the folio text [2]. This is why the
present dossier reports dated AWB names and a bounded negative result rather than inventing a full
annual roll.

### Denkelbuch access boundary, 2026-08-12

A fresh unauthenticated check of the official Rahvusarhiiv AIS record reproduced the catalogue identity for `TLA.230.1.Aa2`, *Ältestes Denkelbuch des Revaler Rats*, including its 1333-1374 date span and 48-page extent [10]. The catalogue HTML exposed metadata only; it did not expose a folio transcription. The official DGS permalink was then fetched directly and ended at the VAU sign-in page. Its redirect parameters identify the protected TIFF target `tla0230_001_0000aa2_00001_x.tif`, but no authenticated image request was made and no manuscript pixels were inspected [11].

The public AIS/gallery preview route was also checked as a discovery fallback. Its anonymous images had no folio/page identifiers or transcription, and the local OCR check recovered no usable council text. This is therefore an **access blocker**, not a negative result about the Denkelbuch: this pass inspected zero folios and cannot establish or refute the December 1343 election outcome, a named burgomaster pair, annual turnover, or the `Johannes de Osenburge` / `Osenbryg` spelling. The clearing condition remains an authenticated VAU/DGS folio review, or an archive-supplied lawful scan/transcription preserving provenance, folio/page identifiers, original wording, and 1340-1345 coverage. Existing access work is tracked by R-451/R-460; no duplicate access task is created here.
### Bunge's secondary annual lists, 1340-1345

Bunge's *Die Revaler Rathslinie* reproduces the older annual council-list tradition in a traceable BSB
scan. The heading says that the names for 1340-1367 represent entry into the relevant office and that
the following number marks exit or last documentary occurrence; Bunge then prints the annual `sitzender
Rat` entries from the underlying list [9]. These are useful as a secondary transcription and as a check on
name forms, but they are not a substitute for reading TLA.230.1.Aa2.

| Year | Bunge's printed `sitzender Rat` list | Reading / use |
|---|---|---|
| **1340** | Hermannus Moren; Reynekinus Crowel; Wennemarus Hollogher; Detmarus de Unna; Rotcherus de Lapide; Gerhardus Stalbiter; Remboldus de Lenepe; Lodwicus Hamer; Hinricus Friso; Gerlacus Caporie; Arnoldus Colner; Arnoldus de Stocken | 12-person active list; first two positions indicate the two leading offices in Bunge's reconstruction [9] |
| **1341** | Winandus Longus; Hermannus Stympel; Wernerus Niger; Theodericus de Unna; Johannes Cracht; Johannes Albus; Hinricus Crowel; Everhardus de Unna; Johannes de Bremen; Thidemannus Cosfelt; Gerlacus Lebart; Constantinus; Hermannus Weldege; Johannes Hamer | 14-person active list; no assumption that every 1340 name left the wider council [9] |
| **1342** | Wennemarus Holloger; Reynekinus Crowel; Rotcherus de Lapide; Detmarus de Unna; Hinricus Friso; Remboldus; Lodwicus Hamer; Everhardus de Unna; Gerhardus Stalbiter; Gerlacus Caporie; Constantinus; Bertoldus Vickynchusen; Hinricus Medebeke | 12-person active list; Bunge says the old and new councils together could show five `proconsules` in this year [9] |
| **1343** | Hermannus Moren; Wynandus Longus; Hermannus Stympel; Theodericus de Unna; Johannes Cracht; Johannes de Bremen; Johannes Albus; Everhardus de Unna; Hinricus Crowel; Thidemannus Cosfelt; Arnoldus Colner; Hermannus Weldege; Johannes Hamer; Johannes Dunevar; Johannes de Lippia | 15-person active list; Bunge says this is 11 names from 1341, two from 1340, and two new names [9] |
| **1344** | Reynekinus Crowel; Wennemarus Holloger; Rotcherus de Lapide; Hinricus Vrese; Gerlacus Caporie; Johannes Albus; Lodwicus Hamer; Remboldus de Lenepe; Gerhardus Stalbiter; Bertoldus Vickynchusen; Wernerus de Calmaria; Hinricus de Beke | 12-person active list; the entry is explicitly dated `Dominica post octavas beati Michaelis` [9] |
| **1345** | Wynandus Longus; Hermannus Moren; Hinricus Crowel; Johannes de Bremen; Thidemannus Cosfelt; Everhardus de Unna; Arnoldus Colner; Johannes Hamer; Hinricus Medebeke; Johannes de Lippia; Johannes Dunevar; followed by six names marked `postea fuerunt electi`: Johannes Albus, Remboldus, Lodwicus Hamer, Rotcherus de Lapide, Bertoldus Vickynchusen, Wernerus de Calmaria | 12 names in the annual entry plus six later elected names; the entry is explicitly dated `Dominica post octavas sancti Michaelis` [9] |

Bunge's structural discussion is as important as the names. He counts 24 as the normal total for the
whole council, with four `proconsules`, and says that annual co-optation normally occurred on the second
Sunday after St Michael (the active session ran from October to October). He also notes that the old and
new councils remained institutionally connected, that 1342 and 1343 could show five burgomasters across
both benches, and that a burgomaster vacancy could be filled immediately rather than waiting for the
annual council change [9]. This explains why the first two names in a sitting list are useful chair
indicators but do not by themselves yield one fixed pair for the whole 1343-1345 interval.

No Bunge page inspected here supplies a separate December 1343 election note. The source therefore
contradicts neither the project's later generic 20 December election description nor the requested
Denkelbuch check, but it does not confirm that date for 1343; keep the December outcome unresolved.


### Dated council members in the accessible AWB evidence

| Name as printed | Dated evidence | Office wording / act | Confidence and use |
|---|---|---|---|
| **Th. Cosfelt** | 1340, AWB 500, printed p. 74 | `camerarius`; acts with Gerlacus Lebart under the town consuls in a homicide security matter | attested office-holder in a 1340 council-managed act; not proven burgomaster [1] |
| **Gerlacus Lebart** | 1340, AWB 500, printed p. 74 | `camerarius`, with Th. Cosfelt | attested chamberlain in this act; not proven burgomaster [1] |
| **Hunoldus de Ostinchusen** | 1341, AWB 504, printed p. 78 | `consul` | attested council member in the dated entry [1] |
| **Remboldus de Lenepe** | 1341, AWB 504, printed p. 78 | `consul` | attested council member in the dated entry [1] |
| **Johannes de Breme** | 1341, AWB 508, printed p. 79 | `nostri consulatus socius`; buys two shoemaker-stall areas beside the consistorium | attested council member/associate in 1341; the same name appears in 1343 and 1345 property acts [1] |
| **Johannes de Osenburge / Osenbryg** | 1341, AWB 523-524, printed pp. 81-82 | town and consuls owe him money; the OCR/print spelling varies | attested named creditor in a council obligation; do not silently normalise the byname [1] |
| **Hermannus More** | 1343, AWB 564 and 566, printed p. 85 | `nostri consulatus socius`; creditor/debtor property transactions | attested council member/associate in 1343 [1] |
| **Wynandus Longus** | 1344, 27 February, AWB 587, printed p. 89 | `nostri consulatus socius` | attested council member/associate in a dated 1344 act [1] |
| **Bertoldus Bickinchusen** | 1344, 10 September, AWB 601, printed p. 91 | `nostri consulatus socius` | attested council member/associate in a dated 1344 act [1] |
| **Reynekinus Crowel** | 1344, AWB 617, printed p. 94 | `nostri consulatus socii`, with Everhardus de Unna and Hermannus More | attested council member/associate in a dated 1344 settlement [1] |
| **Everhardus de Unna** | 1344, AWB 617, printed p. 94 | `nostri consulatus socii` | attested council member/associate in a dated 1344 settlement [1] |
| **Arnoldus** | 1345, AWB 635, printed p. 98 | `nostri consulatus socius`; appears with his brother, a Reval canon | attested council member/associate in a dated 1345 act; forename alone is all the edition supplies here [1] |

The AWB language is specific enough to support dated council membership, but it is not a roster.
Repeated names are therefore useful continuity candidates, not proof of uninterrupted tenure. For
example, **Hermannus More** appears as a council associate in both 1343 and 1344, and **Johannes de
Breme** appears as a council associate in 1341 and in later property acts; continuity across the
interval is **plausible composite** unless the Denkelbuch confirms each year [1][2].

### Civic acts and the council's collective signature

The 1343 diplomatic record supplies dated civic actions but not individual council names:

| Date | Document | What it proves | Confidence |
|---|---|---|---|
| **11 May 1343** | *Diplomatarium Danicum* III:1, no. 321 | Bertram von Parembeke and other Danish officers received a grain levy from the **town of Reval** | attested civic act; does not identify councillors [7] |
| **21 May 1343** | *Diplomatarium Danicum* III:1, no. 324 | `staden Reval` joined an armistice with Swedish representatives and Danish royal councillors in Estonia | attested collective representation; no named burgomaster pair [7] |
| **27 October 1343** | *Diplomatarium Danicum* III:1, no. 376 | `staden Revals borgmestre og rådmænd` petitioned the Livonian Order with bishop, chapter, monasteries, and Danish vassals | attested collective mention of burgomasters and councillors; individual names absent from the regest [7] |
| **7 January 1345** | *Diplomatarium Danicum* III:2, nos. 116-119 | The king confirmed Reval's privileges and authorised the town's councillors and burghers to manage watercourses and mills | attested collective authority; individual names absent [8] |

These documents establish that the council acted as a corporate body during the uprising and that
multiple burgomasters existed by October 1343. They do **not** justify assigning Johannes de Breme,
Hermannus More, or any other AWB witness to a named burgomaster chair without the Denkelbuch or a
full charter witness list [1][7].

### Burgomaster and turnover verdict

- **Dietrich (Didericus) Zierenberg** is the project's existing near-attested burgomaster candidate,
  with a recorded death on 6 December 1345. Retain him as a senior civic face, but cite the existing
  council dossier rather than presenting this dossier as a new primary proof [4][5].
- The **1333 Denkelbuch cohort** (four probable burgomasters and fourteen councillors) is securely
  dated to 1333, but survival, re-election, and chair assignment in 1343-1345 are not demonstrated
  by the accessible AWB pages [3][6].
- **No named 1343-1345 burgomaster pair is established in this pass.** The October 1343 DD regest
  gives the collective phrase `borgmestre og rådmænd`; the accessible AWB excerpts identify council
  associates but do not call any of them `burgimagister` or equivalent [1][7]. This is an
  **attested negative result for the reviewed evidence**, not a claim that the pair cannot be found
  in TLA.230.1.Aa2 or another charter series.
- **Turnover:** the dated AWB sequence shows that the council's active network included Hunoldus de
  Ostinchusen and Remboldus de Lenepe in 1341, Hermannus More in 1343-1344, Wynandus Longus and
  Bertoldus Bickinchusen in 1344, and Arnoldus in 1345. This demonstrates named council activity
  across the period, but not the full rate or cause of turnover. Any replacement narrative is
  `plausible composite` until the election notes are read [1][2].

## Production hooks

- **Character:** Use the dated AWB names as a small, source-labelled pool for council scenes. Keep
  Dietrich Zierenberg as senior burgomaster; use generic burgomaster clothing for the second chair
  rather than inventing a named partner. Mark continuity from one dated AWB entry to another as
  `plausible composite`.
- **Narrative / Quest:** A 1343 council scene can truthfully invoke a collective `Rat`, the grain
  levy of 11 May, the armistice of 21 May, or the October appeal to the Order. A named NPC may be
  present as a council associate only with the relevant AWB date in the writer's note.
- **Dialogue:** Preserve documentary variants (`Johannes de Osenburge` / `Osenbryg`) until a
  manuscript collation resolves them. Do not have a character claim that a named AWB associate is
  `Bürgermeister` unless the source note supplies the chair.
- **Art:** Two civic chairs remain historically required by the collective 1343 wording, but only
  one named burgomaster face is near-attested in the current evidence. Render the second as an
  unnamed senior merchant, not as a fabricated historical person.
- **Dev:** If council members are represented in content data, store `evidence_date`, `source_id`,
  and `office_confidence`; treat `consulatus_socius` as `councillor_associate`, not as an automatic
  `burgomaster` enum.

## Cross-references

- [`town-council-and-officers.md`](town-council-and-officers.md) - existing council structure, 1333 cohort, and Zierenberg brief.
- [`../language/names-address-and-oaths.md`](../language/names-address-and-oaths.md) - documentary name forms and office address.
- [`../power/jurisdictions-of-reval.md`](../power/jurisdictions-of-reval.md) - council, Danish castle, and Order jurisdiction seams.
- [`../economy/awb-clausuris-mortgage-text-1340-1343.md`](../economy/awb-clausuris-mortgage-text-1340-1343.md) - AWB method and direct 1343 OCR citations.

## Open questions

- Read **TLA.230.1.Aa2** folios covering 1340-1345 and transcribe every dated council-election note,
  including the named burgomaster pair and December 1343 election outcome.
- Collate AWB spellings `Johannes de Osenburge` / `Johannes de Osenbryg` against the manuscript or a
  higher-quality edition before merging the name into a canonical character record.
- Check whether Dietrich Zierenberg appears in a dated 1343-1345 charter witness list and identify
  the source behind the reported 6 December 1345 death date.
- Determine whether `camerarius` Th. Cosfelt and Gerlacus Lebart in AWB 500 were sitting councillors,
  alter-Rat officers, or temporary treasury officials in 1340.

## Sources

1. L. Arbusow (ed.), *Das älteste Wittschopbuch der Stadt Reval (1312-1360)*, Reval: Franz Kluge, 1888, Latin/German, public-domain MDZ scan and OCR. Relevant entries: AWB 500 (1340, p. 74), 504 and 508 (1341, pp. 78-79), 523-528 (1341, pp. 81-82), 564-575 (1343, pp. 85-86), 587 (1344, p. 89), 601 (1344, p. 91), 617 (1344, p. 94), 620 (1345, p. 94), and 635-639 (1345, pp. 98-99). Manifest: https://api.digitale-sammlungen.de/iiif/presentation/v2/bsb00149661/manifest
2. International League of Historical Cities / Stadtbücher, **Denkelbuch**, Reval, TLA.230.1.Aa2, 1333-1374, catalogue record and archive reference: https://www.stadtbuecher.de/en/stadtbuecher/estland/kreis-harju/reval-talinn/aeltestes-denkelbuch-des-revaler-rats/ (German/English metadata; folio viewer redirects to RA.ee login).
3. P. Johansen, *Libri de diversis articulis 1333-1374* (Publ. Stadtarchiv Tallinn 8), 1935, the 1333 *Ratsliste* as summarised in the existing council dossier.
4. `../people/town-council-and-officers.md`, existing project dossier, lines 28 and 68-74; records Dietrich (Didericus) Zierenberg as active near 1343 and deceased 6 December 1345.
5. *Liste der Tallinner Stadtoberhäupter*, German Wikipedia, Zierenberg and Reval city-office summary: https://de.wikipedia.org/wiki/Liste_der_Tallinner_Stadtoberh%C3%A4upter (secondary web source; verify against TLA before canon use).
6. `../language/names-address-and-oaths.md`, existing project dossier, 1333 name-stock and evidence boundary.
7. *Diplomatarium Danicum*, 1343, nos. 321, 324, and 376: https://tekstnet.dk/books/dipdan/1343/ (Danish regesta; dates 11 May, 21 May, and 27 October 1343).
8. *Diplomatarium Danicum*, 1345, nos. 116-119: https://tekstnet.dk/books/dipdan/1345/ (Danish regesta; 7 January 1345 Reval privileges and civic water authority).
9. F. G. von Bunge, *Die Revaler Rathslinie nebst Geschichte der Rathsverfassung und einem Anhange über Riga und Dorpat*, Reval: Franz Kluge, 1874, German, BSB digital scan BSB-ID `bsb11190451`, especially pp. 3-4, 21-23, and 37-41. The scan reproduces the annual council-list tradition and Bunge's discussion of `sitzender Rat`, rotation, and `proconsules`; it is secondary evidence, not a folio-level Denkelbuch reading. Manifest: https://api.digitale-sammlungen.de/iiif/presentation/v2/bsb11190451/manifest; page OCR: https://api.digitale-sammlungen.de/ocr/bsb11190451/13 and https://api.digitale-sammlungen.de/ocr/bsb11190451/33 (OCR is discovery support; consult the scan for exact readings).
10. Rahvusarhiiv, **AIS**, description unit `TLA.230.1.Aa2`, official catalogue record: https://ais.ra.ee/en/description-unit/view?id=124030010181&ru=5GsV5p (metadata fetch returned HTML only; no folio transcription exposed, accessed 2026-08-12).
11. Rahvusarhiiv, **DGS/VAU**, `TLA.230.1.Aa2` image permalink: https://www.ra.ee/dgs/_purl.php?shc=TLA.230.1.Aa2 (unauthenticated fetch ended at VAU sign-in and exposed protected TIFF target `tla0230_001_0000aa2_00001_x.tif`; no folio image or text accessed, 2026-08-12). Access-boundary evidence only.

- **Canon Keeper:** approve the distinction between dated AWB council associates and the still-unidentified burgomaster pair before any named second burgomaster enters canon.
- **Producer / Narrative:** keep the Denkelbuch folio read as a follow-up dependency for a fully named 1343-1345 council bench.
