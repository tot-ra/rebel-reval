...
3. **Godot reimport** - `tools/share_character_textures.py --apply` URI-links family maps, then a headless `--import` pass so captures and tests see the new assets without per-body PNG copies.
...
## Fitted outfits and headwear (P0-199)

Human bodies now keep `Clothing_Torso`, `Clothing_Outerwear`, optional
`Clothing_Armor`, sleeves, hose, boots, `Hair_Scalp` and `Hair_Beard` as separate
skinned meshes at every LOD. The face and neck remain in `Character_Head`.
This allows armor to replace a tunic/apron and headwear to hide hair without
replacing the person, animation player or held weapon.

```gdscript
character.equip_wearable(preload("res://assets/characters/shared/hero_mail.tres"))
character.equip_wearable(preload("res://assets/characters/shared/hero_hat_wearable.tres"))
character.unequip_wearable(&"torso") # restores the default work clothes
character.unequip_wearable(&"head")  # restores scalp hair
```

A `CharacterWearable` declares an ID, slot, fitted body, skinned scene and exact
coverage prefixes. Different body proportions require their own fitted mesh;
Kalev's armor is rejected on Mart. Coverage combines across occupied slots and
applies to all distance LODs. `CharacterVariant.wearables` can select an initial
outfit. These are appearance resources; inventory/save data still owns items,
combat effects and persistence. This change does not add armor items to loot.

Open `assets/characters/showcase/modular_character_showcase.tscn` in Godot to
compare work clothes and mail, toggle a cap, switch hammer/sword/unarmed, and
play idle/walk/run/attack. Buttons support mouse, keyboard focus/Enter and
controller D-pad/A. Reproduce the captured plate with `-- --capture`.

See [the improvement and verification report](../../docs/reports/character_modularity_2026-09-05.md).
