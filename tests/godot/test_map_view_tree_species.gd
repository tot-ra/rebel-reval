...
func test_ancient_oak_landmark_is_huge_detailed_oak() -> void:
	...
	assert_true(int(stats["wood_segments"]) >= 600, "giant limbs need smooth multi-section branch paths")
	...
	assert_true(int(stats["terminal_branch_tips"]) >= 180, "the crown needs abundant fine terminal branches")
	assert_true(int(stats["curved_branch_paths"]) >= 150, "hingepuu branches should bend through several sections")
	assert_true(int(stats["interior_branch_junctions"]) >= 90, "branches should emerge from varied places along parent limbs")
	assert_true(int(stats["leaf_count"]) >= 3000, "sacred canopy needs dense leaf sprays")
