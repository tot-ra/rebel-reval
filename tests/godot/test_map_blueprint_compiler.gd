...
func test_compiles_metadata_and_every_explicit_primitive() -> void:
	...
	blueprint.transition(&"exit.east", Rect2i(19, 5, 1, 2), &"other_map", &"entry.west", &"exit.east")
	blueprint.interaction_anchor(&"anchor.work", Vector2i(5, 5), &"work")
	...
	assert_eq(_record_by_id(definition.interaction_anchors, &"anchor.work")["position"], Vector2(176, 176))
