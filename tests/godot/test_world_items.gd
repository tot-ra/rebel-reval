...
## Strip MultiMesh/ShaderMaterial RIDs before free (DEF-006 headless dummy renderer).
func _free_forge(forge: Node) -> void:
	if forge == null or not is_instance_valid(forge):
		return
	MapView3D._strip_geometry_materials(forge)
	forge.free()
