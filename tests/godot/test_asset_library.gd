...
const WOLF_PATH := "res://assets/animals/medieval/medieval_wolf.glb"
...
func test_catalog_lists_imported_models_by_folder() -> void:
	...
	assert_true(paths.has(WOLF_PATH), "medieval wolf should be in the catalog")
...
func test_catalog_filter_matches_name_and_category() -> void:
	var entries: Array[Dictionary] = Catalog.list_models()
	var wolves := Catalog.filter_entries(entries, "wolf", "animals")
	assert_true(wolves.size() >= 1, "wolf filter should return animal models")
	assert_eq(String(wolves[0]["category"]), "animals")
...
func test_library_scene_lists_models_and_can_orbit() -> void:
	...
	assert_true(library.show_model(WOLF_PATH))
	assert_eq(library.current_path(), WOLF_PATH)
	var clips: PackedStringArray = library.current_clips()
	assert_true(clips.size() >= 1, "wolf model should expose animation clips")
