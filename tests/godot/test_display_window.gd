...
func test_target_size_falls_back_for_negative_screen_dimensions() -> void:
	var target := DisplayWindowScript.target_window_size_for_screen(Vector2i(-1280, 720))
	assert_eq(target, Vector2i(1920, 1080), "negative display metrics keep project defaults")
