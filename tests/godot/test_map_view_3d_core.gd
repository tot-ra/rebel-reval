...
func test_water_material_uses_depth_aware_optics() -> void:
	...
	assert_true("star_map" in source, "water must sample the real sky star catalog")
	assert_true("_equatorial_uv" in source, "reflected stars must use the sky astronomical projection")
	assert_true("reflected_sky_ray" in source, "water waves must reflect the view ray into the sky")
