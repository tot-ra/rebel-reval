...
class UpdateTodoCountsTest(unittest.TestCase):
    ...
    def test_bucket_helper_maps_suffixes_and_future_priorities(self) -> None:
        from update_todo_counts import _bucket_for

        self.assertEqual(_bucket_for("P4"), "P4")
        self.assertEqual(_bucket_for("P6"), "P6")
    ...
    def test_rewrite_table_fallback_handles_crlf_line_endings(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            todo = Path(temp_dir) / "TODO.md"
            todo.write_bytes(
                "\r\n".join(
    ...
    def test_cli_check_rejects_missing_summary_without_writing(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            todo = Path(temp_dir) / "custom-todo.md"
            todo.write_text(
                "\n".join(
                    [
                        "# Fixture TODO",
            ...
            self.assertEqual(result.returncode, 1)
            self.assertIn("| P2 |     1  |     0  |", result.stdout)
