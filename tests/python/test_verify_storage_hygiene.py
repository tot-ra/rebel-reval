...
class VerifyStorageHygieneTest(unittest.TestCase):
    ...
    def test_negative_exception_size_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            manifest = root / "docs" / "storage_binary_exceptions.json"
            self._write_manifest(
                ...
                [
                    {
                        "path": "assets/negative.bin",
                        "size_bytes": -1,
                        "sha256": "0" * 64,
                        "owner": "test owner",
                        "rationale": "negative size fixture",
                        "follow_up": "P0-TEST",
                    }
                ],
            )
    ...
    def test_whitespace_only_text_metadata_is_rejected(self) -> None:
        for field in ("owner", "rationale", "follow_up"):
            with self.subTest(field=field), tempfile.TemporaryDirectory() as temp_dir:
                ...
                row = {
                    "path": "assets/large.bin",
                    "size_bytes": verifier.LARGE_FILE_BYTES,
                    "sha256": "0" * 64,
                    "owner": "test owner",
                ...
                row[field] = " \t\n"
                self._write_manifest(root, [row])

                exceptions, errors = verifier.read_exceptions(
                    root / "docs" / "storage_binary_exceptions.json"
                )

            self.assertEqual(exceptions, {})
            self.assertEqual(errors, [f"exception entry 1 missing: {field}"])
