...
from tools.verify_p4_019_central_district_activation import verify


ROOT = Path(__file__).resolve().parents[2]


class TestCentralDistrictActivation(unittest.TestCase):
    ...
    def test_partial_activation_is_rejected(self):
        with self._fixture() as root:
            rrmap = root / "content/maps/market_civic_quarter.rrmap"
            rrmap.write_text(
    ...
    def test_production_activation_requires_approval_and_parity_evidence(self):
        with self._fixture() as root:
            ...
            center = next(row for row in destinations["scenes"] if row["id"] == "reval_center")
            center["release"] = True
    ...
    def test_blocked_state_requires_all_named_dependencies(self):
        with self._fixture() as root:
            ledger_path = root / "docs/data/p4_019_central_district_activation.json"
            ledger = json.loads(ledger_path.read_text(encoding="utf-8"))
            ledger["blockers"].remove("P4-022")
            ledger_path.write_text(json.dumps(ledger), encoding="utf-8")
            errors = verify(root)
            self.assertTrue(any("P4-022" in error for error in errors), errors)
    ...
    def _fixture(self):
        ...
        paths = [
            ...
            "docs/data/p4_019_central_district_activation.json",
