...
- [x] P0-042 | deps: P0-035 | deliverable: deterministic programmatic map-authoring spike for the Smithy Courtyard and adjacent Lower Town street, with reusable grass, sand, hay, dirt, cobblestone, water, and stone terrain treatments plus data-defined buildings, props, and collision footprints; allowed files: `TODO.md`, `docs/MAP_AUTHORING.md`, `scripts/map/*`, `scenes/map_prototype/*`, `tests/godot/test_programmatic_map.gd`; constraints: procedural geometry only, no new or replaced runtime art, no conversion of active districts before P0-040/P2-003 | verify: `godot --headless --path . --script tools/run_godot_tests.gd` passes and running `scenes/map_prototype/smithy_courtyard.tscn` visibly fills the world bounds without gaps, distinguishes all seven terrain IDs, presents a readable street/courtyard/building layout, and blocks every declared building footprint
