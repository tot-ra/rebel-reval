...
WS-11a implementation landed (R-930). `MapViewLighting.apply_cycle_progress` now forwards `sunset_factor` and `water_cloud_darken()` (named preset `darken`, overcast 0.72). Presentation still has no blended `cloud_darken` field; WS-11 can replace the lookup. The R-715 test helper was left unchanged (not in allowed files). `--filter=test_map_view_lighting` 8/8. Harbour plates are a follow-up: a Godot editor was already attached, so GPU captures were skipped.
