...
def render_inventory() -> str:
    ...
    lines.append("")
    lines.append("Generated by `python3 tools/generate_asset_inventory.py` for TODO P0-027.")
    lines.append("")
    lines.append("## Scope")
    lines.append("")
    lines.append("- Includes runtime image and audio source files under `assets/`, `music/`, and `sounds/` with common Godot runtime extensions.")
    lines.append("- Includes `img/` marketing image/audio files only when a Godot `.import` sidecar exists, then classifies them as `archive` unless later evidence promotes them.")
