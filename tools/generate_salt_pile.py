#!/usr/bin/env python3
"""Build a readable game-ready harbour salt pile with Blender.

Run from the repository root:
    blender --background --factory-startup --python tools/generate_salt_pile.py -- --preview

The deterministic rigid prop replaces the egg-like runtime spheres with a coarse
salt mound and an aged-oak grain scoop. Map-owned placement, collision,
...
OUTPUT = ROOT / "assets" / "props" / "crafts" / "salt_pile.glb"
EVIDENCE_DIR = ROOT / "generated" / "blender" / "salt_pile_v1"
DEFAULT_PREVIEW = EVIDENCE_DIR / "preview.png"
BRIEF_PATH = EVIDENCE_DIR / "brief.json"
REPORT_PATH = EVIDENCE_DIR / "report.json"
STATE_PATH = EVIDENCE_DIR / "state.json"
ASSET_ID = "prop.salt_pile"
BLENDER_VERSION = "Blender 5.2 LTS"
GENERATOR_VERSION = "salt_pile_v1"
...
BRIEF = {
    "id": ASSET_ID,
    "kind": "rigid_prop",
    "target": "res://assets/props/crafts/salt_pile.glb",
    "scene": "res://content/maps/lower_town_slice.rrmap#salt_pile",
...
def _create_texture(name: str, base_srgb: tuple[float, float, float], surface: str) -> bpy.types.Image:
    ...
    if surface == "burlap":
        ...
        noise1 = np.sin((u * 40.0 + v * 30.0) * math.tau)
        noise2 = np.sin((u * 80.0 - v * 70.0) * math.tau)
        variation = 0.88 + noise1 * 0.06 + noise2 * 0.06
    else:
...
def _build_salt_mound(materials: list[bpy.types.Material]) -> bpy.types.Object:
    mesh = bmesh.new()
    salt_index = 0
    # Two overlapping ellipsoids for an uneven pile, ground Z is 0
    # Vector3(-0.1, 0.24, 0.06) with radius 0.48 -> Z=0.24, radius Z=0.48, so it intersects ground
    # Godot coords: Y is UP. Blender coords: Z is UP.
    # Godot: Vector3(-0.1, 0.24, 0.06) -> Blender: X=-0.1, Y=0.06, Z=0.24
    _add_ellipsoid(mesh, (-0.1, 0.06, 0.24), (0.48, 0.48, 0.24), salt_index, math.radians(15.0), 16, 8)
    
    # Godot: Vector3(0.2, 0.18, -0.08) -> Blender: X=0.2, Y=-0.08, Z=0.18
    _add_ellipsoid(mesh, (0.2, -0.08, 0.18), (0.36, 0.36, 0.18), salt_index, math.radians(-10.0), 14, 7)
    
    # Add a few smaller clumps around the edges to make it more natural
    _add_ellipsoid(mesh, (0.3, 0.15, 0.08), (0.15, 0.18, 0.08), salt_index, math.radians(45.0), 10, 5)
    _add_ellipsoid(mesh, (-0.35, -0.2, 0.12), (0.22, 0.16, 0.12), salt_index, math.radians(80.0), 12, 6)
    
    return _object_from_bmesh("SaltMound", mesh, materials, smooth=True)
...
def _build_model() -> tuple[bpy.types.Object, list[bpy.types.Object]]:
    materials = [
        _create_material("CoarseSalt", (0.88, 0.88, 0.85), 0.75, "salt"),
        _create_material("AgedOakGrainScoop", WOOD_SRGB, 0.88, "wood"),
    ]
    root = bpy.data.objects.new("SaltPile", None)
    bpy.context.collection.objects.link(root)

    salt = _build_salt_mound(materials)
...
def _write_evidence(metrics: dict[str, object], preview: Path | None) -> None:
    ...
    report = {
        **metrics,
        ...
        "generator": "tools/generate_salt_pile.py",
        ...
        "checks": {
            ...
            "identity_features": ["coarse_salt_mound", "grain_scoop"],
        },
    }
    ...
    if (EVIDENCE_DIR / "godot_preview.png").exists():
        report["godot_preview"] = "generated/blender/salt_pile_v1/godot_preview.png"
