#!/usr/bin/env python3
"""Build three game-ready medieval hand tools with deterministic Blender geometry.
...
Each asset is independently placeable: forge tongs, a three-tine pitchfork, and a
short-bladed medieval grass scythe.
"""
...
GENERATOR_VERSION = "medieval_hand_tools_v1"
TEXTURE_SIZE = 512
...
def _build(spec: AssetSpec, preview_enabled: bool) -> dict[str, object]:
    ...
    if spec.slug == "blacksmith_tongs":
        objects = _build_tongs(root, materials)
        identity = "riveted two-arm blacksmith tongs with short flat gripping jaws"
    elif spec.slug == "pitchfork":
        objects = _build_pitchfork(root, materials)
        identity = "three hand-forged tines socketed onto a long ash shaft"
    else:
        objects = _build_scythe(root, materials)
        identity = "curved ash snath, side grip, iron socket, and short grass-cutting blade"
