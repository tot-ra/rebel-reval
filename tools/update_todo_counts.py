...
def _bucket_for(priority: str) -> str:
    if priority in PRIORITY_MAP:
        return priority
    # Strip the task number and optional letter suffix (e.g. "P4-027a" → "P4").
    base_match = re.match(r"^(D|R|A|P\d+)-\d+[a-z]*$", priority)
    base = base_match.group(1) if base_match else priority
    return base if base in PRIORITY_MAP else OTHER_PREFIX
