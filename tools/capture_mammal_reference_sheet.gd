...
const OUTPUT := "res://docs/reports/images/fauna/p0_118_mammal_reference_sheet.png"
const VIEWPORT_SIZE := Vector2i(1800, 1200)
const COLUMNS := 6
const CELL_SIZE := Vector2(2.8, 2.35)
const DISPLAY_TARGET := 1.55
...
func _run() -> void:
	...
	for index in MammalSpecies.ALL_SPECIES.size():
		var species: StringName = MammalSpecies.ALL_SPECIES[index]
		var column := index % COLUMNS
		var row := index / COLUMNS
		var origin := Vector3(
			(float(column) - float(COLUMNS - 1) * 0.5) * CELL_SIZE.x,
			(float((MammalSpecies.ALL_SPECIES.size() - 1) / COLUMNS) * 0.5 - float(row)) * CELL_SIZE.y,
			0.0
		)
		viewport.add_child(_mammal_entry(species, origin))

	var camera := Camera3D.new()
	camera.projection = Camera3D.PROJECTION_ORTHOGONAL
	camera.size = 14.4
	camera.position = Vector3(0.0, 0.35, 24.0)
	...
	if error != OK:
		push_error("Could not save mammal reference sheet %s: %s" % [OUTPUT, error_string(error)])
		quit(1)
		return
	print("P0-118 mammal reference sheet: %s" % OUTPUT)
	viewport.queue_free()
	quit(0)


func _mammal_entry(species: StringName, origin: Vector3) -> Node3D:
	...
	instance.position = Vector3(0.0, 0.12 - (bounds.position.y + bounds.size.y * 0.5) * visual_scale, 0.0)
	instance.rotation_degrees = Vector3(-8.0, -24.0, 0.0)
	...
	label.name = "Label"
	label.text = String(MammalSpecies.id_for(species))
	label.font_size = 24
	label.modulate = Color("e9e2d2")
	label.outline_size = 5
	label.outline_modulate = Color("202527")
	label.position = Vector3(0.0, -0.92, 0.10)
