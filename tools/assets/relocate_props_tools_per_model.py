#!/usr/bin/env python3
"""Colocate each medieval hand-tool GLB with its Godot sidecars under assets/props/tools/<slug>/."""
...
def collect_moves() -> list[tuple[Path, Path]]:
    ...
    for path in sorted(TOOLS.iterdir()):
        ...
        if model is None:
            raise SystemExit(f"Unassigned tools root file: {path.name}")
        dest = TOOLS / model / path.name
...
def rewrite_import_sidecars(moves: list[tuple[Path, Path]]) -> None:
    for _src, dest in moves:
        ...
        old = f"res://assets/props/tools/{asset_name}"
        new = f"res://assets/props/tools/{dest.parent.name}/{asset_name}"
        if old not in text:
            continue
...
def build_replacements(moves: list[tuple[Path, Path]]) -> list[tuple[str, str]]:
    ...
    for src, dest in moves:
        ...
        reps.append((f"res://assets/props/tools/{name}", f"res://assets/props/tools/{model}/{name}"))
        reps.append((f"assets/props/tools/{name}", f"assets/props/tools/{model}/{name}"))
...
def moves_from_layout() -> list[tuple[Path, Path]]:
    moves: list[tuple[Path, Path]] = []
    for model_dir in sorted(p for p in TOOLS.iterdir() if p.is_dir()):
