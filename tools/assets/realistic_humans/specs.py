...
# --- Crowd ---------------------------------------------------------------
# The P0-153 seeded roster (assets/characters/variants/crowd_variation_manifest.json,
# generated by tools/character_specs.py) stays the source of truth: each seeded
# body is realised here as a MakeHuman spec with the same stature order, build,
# skin tone, hair, gait (variant resource) and period dyes.

_CROWD_DYES = {
    "undyed_grey": (0.46, 0.44, 0.40), "undyed_brown": (0.36, 0.28, 0.20),
    "woad_blue": (0.27, 0.35, 0.46), "madder_red": (0.52, 0.25, 0.18),
    "weld_yellow": (0.60, 0.52, 0.28), "walnut_brown": (0.30, 0.21, 0.14),
    "woad_weld_green": (0.35, 0.40, 0.28), "bleached_linen": (0.84, 0.82, 0.76),
    "raw_linen": (0.72, 0.67, 0.56), "tan": (0.50, 0.35, 0.21), "dark": (0.28, 0.20, 0.14),
    "oiled": (0.38, 0.26, 0.16),
}
_CROWD_HAIR = {"flaxen": (0.62, 0.53, 0.36), "dark_brown": (0.24, 0.18, 0.13), "auburn": (0.46, 0.25, 0.15),
               "brown": (0.34, 0.25, 0.17), "black": (0.12, 0.10, 0.09), "grey": (0.55, 0.53, 0.50)}


def _crowd_spec(body):
    import random
    rng = random.Random(f"{body['template']}:{body['seed']}:realistic")
    male = body["template"] == "townsman"
    features, dyes = body["features"], body["dyes"]
    build = body["build_factor"]
    age = rng.randint(20, 58)
    skin = ("young" if age < 32 else "middleage" if age < 50 else "old") + ("_caucasian_male" if male else "_caucasian_female")
    hair_color = _CROWD_HAIR.get(body["hair"], (0.3, 0.22, 0.15))
    tone = body["skin_tone"]
    palette = {"hose": _CROWD_DYES[dyes["pants"]], "boots": _CROWD_DYES[dyes["boots"]],
               "hood": _CROWD_DYES[dyes["accent"]]}
    if male:
        torso = "wool_tunic" if features["tunic_length"] == "long" else "short_tunic"
        palette[torso] = _CROWD_DYES[dyes["tunic"]]
        garments = [torso, "hose", "boots"] + (["hood"] if rng.random() < 0.4 else [])
    else:
        torso = "gown" if features["outerwear"] == "kirtle" or features["tunic_length"] == "long" else "work_gown"
        palette[torso] = _CROWD_DYES[dyes["tunic"]]
        palette["headscarf"] = _CROWD_DYES[dyes["sleeves"]]
        garments = [torso, "headscarf", "boots"]
        if features["outerwear"] == "kirtle":
            garments.insert(1, "waist_apron")
    spec = _person(
        age=age, sex="m" if male else "f", muscle=0.45 + 0.2 * rng.random(),
        weight=min(0.85, max(0.3, 0.5 + (build - 1.0) * 1.6)),
        height_m=(1.72 if male else 1.61) * body["stature_factor"], skin=skin,
        eyes=rng.choice(["blue", "grey", "bluegreen", "lightblue", "brown", "green"]),
        hair=None if features["hair_style"] == "bald" else rng.choice(["short01", "short04"] if male else ["bob01", "braid01"]),
        hair_color=hair_color, brows=rng.choice(["eyebrow001", "eyebrow002", "eyebrow004"] if male else ["eyebrow006"]),
        lashes="eyelashes02" if male else "eyelashes03",
        targets={"nose-hump-incr": round(rng.random() * 0.5, 2), "chin-width-incr": round(rng.random() * 0.4, 2),
                 "head-square" if male else "head-oval": round(rng.random() * 0.4, 2),
                 "eyebrows-trans-down": round(rng.random() * 0.3, 2)},
        complexion={"tan": 0.3 + 0.4 * rng.random(), "flush": 0.3 + 0.3 * rng.random(),
                    "skin_tint": [0.78 + 0.3 * (tone - 0.5)] * 3},
        beard=({"length": 0.006 + 0.006 * rng.random(), "color": hair_color, "grey": 0.3 if age > 45 else 0.05,
...
def _load_crowd():
    ...
    manifest = Path(__file__).resolve().parents[3] / "assets/characters/variants/crowd_variation_manifest.json"
    if not manifest.is_file():
        return {}
    return {body["name"]: _crowd_spec(body) for body in json.loads(manifest.read_text())["bodies"]}


for _name, _spec in _load_crowd().items():
    SPECS[_name] = _spec
    _spec.setdefault("fit", _name)
    _spec.setdefault("stable_id", f"char.{_name}")
