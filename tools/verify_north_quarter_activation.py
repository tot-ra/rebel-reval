#!/usr/bin/env python3
"""Fail-closed activation readiness gate for the North Quarter.

P4-020 cannot promote one registry in isolation: the rrmap declaration, map
catalog, transition manifest, approval ledger, and visual evidence must agree.
The inactive state is also checked so dependency-blocked work cannot drift into
an accidental partial release.
"""
...
RRMAP_PATH = Path("content/maps/north_quarter.rrmap")
...
def _rrmap_state(path: Path) -> tuple[str, bool]:
    for line in path.read_text(encoding="utf-8").splitlines():
        if not line.startswith("map north_quarter "):
            continue
        scope = re.search(r"\bscope=(\w+)", line)
        active = re.search(r"\bactive=(true|false)", line)
        if scope and active:
            return scope.group(1), active.group(1) == "true"
    raise ValueError("north_quarter map declaration must include scope and active")


def _catalog_state(path: Path) -> tuple[str, bool]:
    text = path.read_text(encoding="utf-8")
    match = re.search(
        r'"reval_north"\s*:\s*\{(?P<body>.*?)\}', text, re.DOTALL
    )
    if not match:
        raise ValueError("map catalog is missing reval_north")
    body = match.group("body")
    scope = re.search(r'"scope"\s*:\s*"([^"]+)"', body)
    active = re.search(r'"active"\s*:\s*(true|false)', body)
    if not scope or not active:
        raise ValueError("reval_north catalog entry must include scope and active")
    return scope.group(1), active.group(1) == "true"
...
def _destination_state(path: Path) -> tuple[bool, bool]:
    ...
    matches = [
        row for row in data.get("scenes", []) if row.get("id") == "reval_north"
    ]
    if len(matches) != 1:
        raise ValueError("transition manifest must contain exactly one reval_north entry")
    row = matches[0]
...
def verify(root: Path) -> list[str]:
    ...
    if all(production_states):
        ...
        if (rrmap_scope, rrmap_active) != ("prototype", False):
            errors.append(
                "blocked North Quarter rrmap must remain scope=prototype active=false"
            )
        if (catalog_scope, catalog_active) != ("prototype", False):
            errors.append("blocked reval_north catalog entry must remain prototype/inactive")
        if not destination_active or destination_release:
            errors.append("blocked reval_north must remain active developer traversal with release=false")
        if decision != "blocked":
...
def main() -> int:
    ...
    if errors:
        for error in errors:
            print(f"ERROR: {error}")
        return 1
    print("P4-020 North Quarter activation readiness guard passed.")
    return 0
