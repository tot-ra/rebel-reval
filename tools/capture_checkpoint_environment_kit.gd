...
## rather than relying on a whole-district calibration plate. The map and view
## remain authoritative; this script changes only the evidence camera.
...
## Focus covers the Viru gate throat, both gate towers, the gate cart, and the
## incomplete foregate context while retaining the playable approach lane.
const CHECKPOINT_FOCUS_CELL := Vector2(117.5, 55.5)
const CHECKPOINT_FOCUS_HEIGHT := 0.8
const CHECKPOINT_CAMERA_SIZE := 17.5
...
func _run() -> void:
	...
	if definition.map_id != StringName(MAP_ID):
		push_error("Unexpected map id for checkpoint evidence capture: %s" % definition.map_id)
		quit(1)
		return
	for time_of_day in MapView3D.ALL_TIMES:
		...
		if error != OK:
			...
			return
	print(
		"P0-102m.4 checkpoint evidence written: %s/%s_{day,night}.png"
...
func _capture(definition: MapDefinition, time_of_day: StringName) -> Error:
	...
	_configure_checkpoint_camera(view, definition)

	for _frame in 12:
		await process_frame
	var texture := viewport.get_texture()
	if texture == null:
		push_error("Checkpoint evidence viewport has no texture for %s" % time_of_day)
		viewport.queue_free()
		await process_frame
		return ERR_CANT_CREATE
	var image := texture.get_image()
	var output := "%s/checkpoint_%s.png" % [OUTPUT_DIR, time_of_day]
	var error := image.save_png(ProjectSettings.globalize_path(output))
	if error != OK:
		push_error("Could not save checkpoint evidence %s: %s" % [output, error_string(error)])
	else:
		print("Checkpoint evidence: %s (%dx%d)" % [output, image.get_width(), image.get_height()])
	viewport.queue_free()
	await process_frame
	return error


func _configure_checkpoint_camera(view: MapView3D, definition: MapDefinition) -> void:
	var camera := view.view_camera()
	if camera == null or camera.projection != Camera3D.PROJECTION_ORTHOGONAL:
		push_error("Checkpoint evidence requires MapView3D's orthographic camera")
		return
	camera.size = CHECKPOINT_CAMERA_SIZE
	var focus := MapViewBridge.logic_to_world(
		CHECKPOINT_FOCUS_CELL * float(definition.cell_size),
		definition.cell_size,
		CHECKPOINT_FOCUS_HEIGHT
	)
